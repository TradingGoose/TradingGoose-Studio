...
export const systemBillingTier = pgTable(
  'system_billing_tier',
  {
    ...
    workflowExecutionTimeLimitSeconds: decimal('workflow_execution_time_limit_seconds'),
  ...
  (table) => ({
    ...
    workflowExecutionTimeLimitCheck: check(
      ...
      sql`${table.workflowExecutionTimeLimitSeconds} is null or (${table.workflowExecutionTimeLimitSeconds} > 0 and ${table.workflowExecutionTimeLimitSeconds} <> 'NaN'::numeric and ${table.workflowExecutionTimeLimitSeconds} <> 'Infinity'::numeric and ${table.workflowExecutionTimeLimitSeconds} <> '-Infinity'::numeric)`
    ),
    statusCheck: check(
