...
export const systemBillingSettings = pgTable(
  ...
  {
    id: text('id').primaryKey(),
    onboardingAllowanceUsd: decimal('onboarding_allowance_usd').notNull().default('0'),
    overageThresholdDollars: decimal('overage_threshold_dollars').notNull().default('50'),
    workflowExecutionChargeUsd: decimal('workflow_execution_charge_usd').notNull().default('0'),
    functionExecutionChargeUsd: decimal('function_execution_charge_usd').notNull().default('0'),
    usageWarningThresholdPercent: integer('usage_warning_threshold_percent').notNull().default(80),
    freeTierUpgradeThresholdPercent: integer('free_tier_upgrade_threshold_percent')
      .notNull()
      .default(90),
  ...
  (table) => ({
    ...
    updatedByUserIdIdx: index('system_billing_settings_updated_by_user_id_idx').on(
...
export const systemBillingTier = pgTable(
  'system_billing_tier',
  {
    ...
    status: text('status').$type<SystemBillingTierStatus>().notNull().default('draft'),
    ownerType: text('owner_type').$type<SystemBillingTierOwnerType>().notNull(),
    usageScope: text('usage_scope').$type<SystemBillingTierUsageScope>().notNull(),
    seatMode: text('seat_mode').$type<SystemBillingTierSeatMode>().notNull().default('fixed'),
    ...
    workflowExecutionTimeLimitSeconds: integer('workflow_execution_time_limit_seconds'),
    ...
    apiEndpointRateLimitPerMinute: integer('api_endpoint_rate_limit_per_minute'),
    ...
    workflowExecutionMultiplier: decimal('workflow_execution_multiplier').notNull().default('1'),
    workflowModelCostMultiplier: decimal('workflow_model_cost_multiplier').notNull().default('1'),
    functionExecutionMultiplier: decimal('function_execution_multiplier').notNull().default('0'),
    copilotCostMultiplier: decimal('copilot_cost_multiplier').notNull().default('1'),
  ...
  (table) => ({
    ...
    displayOrderIdx: index('system_billing_tier_display_order_idx').on(table.displayOrder),
    accessCodeUnique: uniqueIndex('system_billing_tier_access_code_unique').on(table.accessCode),
    stripeMonthlyPriceIdUnique: uniqueIndex(
      'system_billing_tier_stripe_monthly_price_id_unique'
    ).on(table.stripeMonthlyPriceId),
    stripeYearlyPriceIdUnique: uniqueIndex('system_billing_tier_stripe_yearly_price_id_unique').on(
      table.stripeYearlyPriceId
    ),
    stripeProductIdUnique: uniqueIndex('system_billing_tier_stripe_product_id_unique').on(
