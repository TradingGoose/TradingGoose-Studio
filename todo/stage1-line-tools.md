...
- Limit import rewrites to `*.ts`/`*.tsx` source files only (do not touch README/rollup.config.js/typedoc.json/package.json).
...
- Implement a plugin adapter under `apps/tradinggoose/widgets/widgets/data_chart/drawings/` that:
  ...
  - Supports manual tool lifecycle on chart panes with capability-based behavior (no per-tool hardcoded overrides): creation for all registered tools, selection/edit actions when hittable/editable, and id-based remove for non-selectable tools.
...
1. **Line-tools location + naming**
   ...
   - Transition note: current repo tree is still pre-rename (`plugins/lightweight-charts-line-tools-*`); Detailed Step 1 performs the rename to the locked mapping below.
...
2. **Module resolution**
   ...
   - Rewrite package-name imports (e.g., `from 'lightweight-charts-line-tools-core'`) inside vendored tools to local relative paths computed from each source file depth (for example, `plugins/<tool>/src/index.ts` -> `../../core/src`, `plugins/<tool>/src/model/*.ts` -> `../../../core/src`), since upstream packages import core by name and will not resolve without aliases.
   - Restrict rewrites to TS/TSX sources; leave README/rollup/typedoc/package.json untouched to avoid noise.
   - If a single “types” entrypoint is needed, re-export from the local vendored core path (for example `apps/tradinggoose/widgets/widgets/data_chart/plugins/core/src/types`) rather than any vendored package-name import.
...
3. **Upstream source + pin (line-tools v1.0.0)**
   - Source repos (difurious), pinned to the local snapshot in `../lightweight-charts-tools/*` at the start of Stage 1. Do not pull/update during Stage 1.
   ...
   - Compatibility: verified via upstream `package.json` `peerDependencies` in `../lightweight-charts-tools/*/package.json` declaring `lightweight-charts` `^5.0.0`, so `5.1.0` is supported; no version alignment required.
...
4. **Plugin API usage**
   ...
   - `seriesAttachmentKey` contract: resolved pane/series attachment identity string (`price:${mainSeriesIdentity}` or `indicator:${indicatorId}:anchor:${paneAnchorIdentity}`).
   - One plugin instance per `seriesAttachmentKey` (not per owner).
   - Plugin uniqueness is global by `seriesAttachmentKey` across both ownership domains (`drawToolsByManual` and `drawToolsByIndicator`): the same `seriesAttachmentKey` must not create two plugin instances.
   - Multiple owners may target the same pane/series and must share the same plugin instance for that target.
   - Keep a single global plugin-instance registry (`pluginsBySeriesAttachmentKey`) plus owner-to-series-attachment bindings + per-series-attachment refcount so shared plugin lifecycle is deterministic.
...
6. **Tool registration set (v1)**
   ...
   - Capability-detection rule (locked, no per-tool hardcoding):
     - After each create/update, parse `getLineToolByID(id)` and persist `canEdit = export.options?.editable !== false` in owner-scoped metadata.
...
7. **Manual draw-tools UI (locked)**
   ...
   - Every registered manual tool must be selectable from the sidebar.
   ...
   - Tool button selection calls `startManualTool(selectedType, drawToolsOwnerId)` for the active manual owner.
...
- `apps/tradinggoose/widgets/widgets/data_chart/plugins/core/src/api/public-api.ts`
- `apps/tradinggoose/widgets/widgets/data_chart/plugins/core/src/core-plugin.ts`
- `apps/tradinggoose/widgets/widgets/data_chart/plugins/core/src/interaction/interaction-manager.ts`
- `apps/tradinggoose/widgets/widgets/data_chart/plugins/core/src/index.ts`
...
## Detailed steps
1. **Vendor line-tools**
   - Vendor from the local snapshot in Decision #3; compatibility with `lightweight-charts` 5.1.0 is already confirmed and no alignment is required.
   - Verify the existing vendored snapshot under `apps/tradinggoose/widgets/widgets/data_chart/plugins/**` against `../lightweight-charts-tools/*` using this allowlist:
   ...
   - Rewrite imports in vendored tools that reference `lightweight-charts-line-tools-core` to local relative paths computed from each file depth (for example `plugins/<tool>/src/index.ts` -> `../../core/src`, `plugins/<tool>/src/model/*.ts` -> `../../../core/src`).
   - Only rewrite TS/TSX source files; leave README/rollup/typedoc/package.json as-is.
   ...
   - If a types-only import surface is needed, re-export types from the local vendored core path in local app code (for example `apps/tradinggoose/widgets/widgets/data_chart/plugins/core/src/types`) without adding files under vendored `plugins/**` and without package-name imports.
   ...
   - After import rewrites + teardown patch, divergence from raw upstream snapshot is expected and must remain within the allowlist above.
   - Teardown patch targets (explicit):
     - `.../src/api/public-api.ts`: add `destroy(): void` to `ILineToolsApi`.
     - `.../src/core-plugin.ts`: implement `destroy()` for plugin-level cleanup.
     - `.../src/interaction/interaction-manager.ts`: implement `destroy()` that unsubscribes/removes all bound listeners.
     - `.../src/index.ts`: update `createDummyPluginApi()` to expose no-op `destroy()`.
   - Keep the vendored code untouched where possible to reduce future diff noise (limit edits to required import path rewrites + teardown patch).
...
3. **Create the plugin adapter**
   ...
   - Maintain a single global plugin-instance registry keyed by `seriesAttachmentKey`:
     - `pluginsBySeriesAttachmentKey` -> plugin instance (global uniqueness across domains).
   ...
   - Provide `attach`/`detach` APIs keyed by `drawToolsOwnerId`, sourced from `manual:${params.view.drawTools[].id}`. Resolve `seriesAttachmentKey` from `drawTools[].pane` + runtime identity:
   ...
   - Add mandatory owner-target reconciliation to prevent stale plugin instances when series anchors change:
     ...
     - Build `seriesAttachmentKey` as:
       - `price:${mainSeriesIdentity}` for `pane === 'price'`.
       - `indicator:${indicatorId}:anchor:${paneAnchorIdentity}` for `pane === 'indicator'`.
...
5. **Add drawTools view storage**
   ...
   - Add `draw-control.tsx` to pane overlay rendering in `chart-body.tsx`:
     ...
     - Position it immediately left of existing `PaneControl`.
...
6. **Align Stage 2 docs**
   - Update `todo/stage2-draw.md` (and any related docs) to clarify that Stage 2 indicator `draw.*` rendering is separate from Stage 1 manual `drawTools` ownership.
   - Keep manual `drawTools` state untouched by Stage 2 indicator rendering.
...
- Typecheck gate (high-baseline, diff-based):
  ...
  - Capture pre-change diagnostics once for this branch before Stage 1 implementation (`cd apps/tradinggoose && bun run type-check -- --pretty false`).
  - Run the same Bun command after Stage 1 changes and compare diagnostics to pre-change output.
  - Path-model transition rule for diagnostics:
    - Pre-rename checks reference current paths under `plugins/lightweight-charts-line-tools-*`.
    - Post-rename checks reference target paths under `plugins/*` (for core: `plugins/core`).
    - Treat pre/post paths as equivalent by Decision #1 mapping when evaluating net-new diagnostics.
  - Stage 1 acceptance requires:
    ...
    - zero new diagnostics in Stage 1 touched paths (`data_chart/drawings/**`, `data_chart/components/chart-body.tsx`, `data_chart/components/draw-tools-sidebar.tsx`, `data_chart/components/draw-control.tsx`, `data_chart/components/draw-tool-icon-registry.ts`, `data_chart/hooks/use-indicator-sync.ts`, `data_chart/types.ts`, and all vendored TS/TSX files changed by Stage 1 rewrites/patches under `data_chart/plugins/**`), including:
      - vendored core teardown patch targets in either pre-rename `plugins/lightweight-charts-line-tools-core/**` or post-rename `plugins/core/**` paths;
      - non-core vendored tool package source/entry TS/TSX files touched by import-path rewrites in either pre-rename `plugins/lightweight-charts-line-tools-*/**` or post-rename `plugins/*/**` paths.
- Build/resolve check to ensure local vendored imports (including rewritten core paths) resolve.
- Reproducibility check:
  - Verify vendored sources under `apps/tradinggoose/widgets/widgets/data_chart/plugins/**` are tracked in repo for this branch (no untracked required vendored files).
- Load `data_chart` and verify manual drawing flow:
  ...
  - Every registered tool is selectable from sidebar (direct button or dropdown item), and sidebar buttons are icon-only.
  - `Lines` and `Freehand` family buttons open hover dropdowns and allow selecting sub-tools.
  - `clearAll` is the last sidebar button.
  - Start each tool type from the sidebar; verify creation for all registered tools and selection/edit behavior for tools that are hittable/editable.
  - Selecting a manual drawing in a pane shows `draw-control` immediately left of `PaneControl` for that pane.
  ...
  - For tools where `canEdit === false`, verify non-selection remove uses locked single-instance-per-owner+type policy (create replaces prior instance; sidebar remove deletes the tracked instance only).
...
- Vendored folder normalization is not yet applied: non-core tool folders still need to be renamed to prefixless `plugins/*` names per Decision #1.
- Current branch state: `apps/tradinggoose/widgets/widgets/data_chart/plugins/**` is still untracked; Stage 1 reproducibility gate is not satisfied until vendored sources are committed.
...
- Vendored core teardown API (`destroy`) is not yet implemented; lifecycle cleanup remains incomplete until that patch lands.
  - Pre-rename current paths (actual tree today):
    - `apps/tradinggoose/widgets/widgets/data_chart/plugins/lightweight-charts-line-tools-core/src/api/public-api.ts` still lacks `destroy(): void` on `ILineToolsApi`.
    - `apps/tradinggoose/widgets/widgets/data_chart/plugins/lightweight-charts-line-tools-core/src/core-plugin.ts` still lacks concrete `destroy()` implementation on `LineToolsCorePlugin`.
    - `apps/tradinggoose/widgets/widgets/data_chart/plugins/lightweight-charts-line-tools-core/src/interaction/interaction-manager.ts` still lacks listener-unsubscribe `destroy()` implementation.
    - `apps/tradinggoose/widgets/widgets/data_chart/plugins/lightweight-charts-line-tools-core/src/index.ts` `createDummyPluginApi()` still lacks no-op `destroy()`.
  - Post-rename target paths for the same required patch set:
    - `apps/tradinggoose/widgets/widgets/data_chart/plugins/core/src/api/public-api.ts`
    - `apps/tradinggoose/widgets/widgets/data_chart/plugins/core/src/core-plugin.ts`
    - `apps/tradinggoose/widgets/widgets/data_chart/plugins/core/src/interaction/interaction-manager.ts`
    - `apps/tradinggoose/widgets/widgets/data_chart/plugins/core/src/index.ts`
...
## Assumptions
- Local `../lightweight-charts-tools/*` snapshot exists and is stable for Stage 1.
- No pulls/updates occur during Stage 1 (per Decision #3).
- `lightweight-charts` remains `5.1.0` during Stage 1.
- `drawTools[].id` values are unique per chart instance; `drawToolsOwnerId` is derived from `params.view.drawTools[].id`.
- Owner-id namespaces stay disjoint across domains (`manual:*` for Stage 1, `indicator:*` for Stage 2).

## ID model (single source of truth)
- `drawTools.id` is persisted in `view.drawTools[]` for manual ownership references.
- `drawToolsOwnerId` is runtime owner key only: `manual:<drawTools.id>` or `indicator:<indicatorId>`.
- `seriesAttachmentKey` is runtime pane/series attachment key string: `price:${mainSeriesIdentity}` or `indicator:${indicatorId}:anchor:${paneAnchorIdentity}`.
- `pluginsBySeriesAttachmentKey` is the single global plugin-instance registry keyed by `seriesAttachmentKey`.
