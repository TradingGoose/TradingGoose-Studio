...
- `apps/tradinggoose/providers/trading/robinhood/` adds the trading adapter. `getRobinhoodTradingAccounts` exposes only Agentic-enabled accounts; `getRobinhoodTradingAccountSnapshot` paginates positions defensively; `submitRobinhoodOrder` reviews only explicit previews, sends confirmed submissions directly to placement, derives a stable `ref_id`, and normalizes structured dollar amounts; `robinhoodOrderDetailRequest` loads persisted orders through the owning account.
