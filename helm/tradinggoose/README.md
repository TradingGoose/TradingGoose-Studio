# TradingGoose Helm Chart

This Helm chart deploys TradingGoose, a lightweight AI agent workflow platform, on Kubernetes.
...
helm install tradinggoose ./helm/tradinggoose
```
...
```bash
helm install tradinggoose ./helm/tradinggoose -f custom-values.yaml
```
...
```bash
helm install tradinggoose-dev ./helm/tradinggoose \
  --values ./helm/tradinggoose/examples/values-development.yaml \
  --namespace tradinggoose-dev --create-namespace
```

### Production Environment

```bash
helm install tradinggoose-prod ./helm/tradinggoose \
  --values ./helm/tradinggoose/examples/values-production.yaml \
  --namespace tradinggoose-prod --create-namespace
```

### Azure Environment

```bash
helm install tradinggoose-azure ./helm/tradinggoose \
  --values ./helm/tradinggoose/examples/values-azure.yaml \
  --namespace tradinggoose --create-namespace
```

### AWS Environment (EKS)

```bash
helm install tradinggoose-aws ./helm/tradinggoose \
  --values ./helm/tradinggoose/examples/values-aws.yaml \
  --namespace tradinggoose --create-namespace
```
...
```bash
helm install tradinggoose-gcp ./helm/tradinggoose \
  --values ./helm/tradinggoose/examples/values-gcp.yaml \
  --namespace tradinggoose --create-namespace
```
...
```bash
helm install tradinggoose-prod ./helm/tradinggoose \
  --values ./helm/tradinggoose/examples/values-external-db.yaml \
  --set externalDatabase.host="your-rds-endpoint.com" \
  --set externalDatabase.username="tradinggoose_user" \
  --set externalDatabase.password="secure-password" \
  --set externalDatabase.database="tradinggoose_prod" \
  --namespace tradinggoose --create-namespace
```
...
| `global.useRegistryForAllImages` | Use custom registry for all images (not just tradinggoose/*) | `false` |
...
| `app.image.repository` | App image repository | `tradinggoose/tradinggoose` |
...
| `postgresql.auth.database` | PostgreSQL database name | `tradinggoose` |
...
| `externalDatabase.database` | External database name | `tradinggoose` |
...
| `ingress.app.host` | App ingress hostname | `tradinggoose.local` |
| `ingress.app.paths` | App ingress paths | `[{path: "/", pathType: "Prefix"}]` |
| `ingress.realtime.host` | Realtime ingress hostname | `tradinggoose-ws.local` |
| `ingress.realtime.paths` | Realtime ingress paths | `[{path: "/", pathType: "Prefix"}]` |
| `ingress.tls.enabled` | Enable TLS for ingress | `false` |
| `ingress.tls.secretName` | TLS secret name | `tradinggoose-tls-secret` |
...
| `migrations.image.repository` | Migrations image repository | `tradinggoose/migrations` |
...
- Automatic service discovery for TradingGoose components
...
- Allow communication between TradingGoose components
...
```bash
helm upgrade tradinggoose ./helm/tradinggoose
```
...
- `API_ENCRYPTION_KEY`: Encrypts API keys at rest in database (must be exactly 64 hex characters). Generate using: `openssl rand -hex 32` (outputs 64 hex chars representing 32 bytes)
...
ingress:
  ...
  tls:
    enabled: true
    secretName: "tradinggoose-tls-secret"
```
