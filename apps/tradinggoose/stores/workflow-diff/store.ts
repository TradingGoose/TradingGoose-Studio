      ...
      return {
        ...
        setProposedChanges: async (
          ...
          if (result.success && result.diff) {
            ...
            try {
              const { getCopilotStore } = await import('@/stores/copilot/store')
              const { messages } = getCopilotStore().getState() as any
        ...
        acceptChanges: async () => {
          ...
          const { getActiveWorkflowId, getLoadedChannelsForWorkflow } =
          ...
          try {
            ...
            try {
              const { getCopilotStore } = await import('@/stores/copilot/store')
              const { currentChat } = getCopilotStore().getState() as any
            ...
            ;(async () => {
              ...
              try {
                const { getCopilotStore } = await import('@/stores/copilot/store')
                const { messages, toolCallsById } = getCopilotStore().getState()
                ...
                if (toolCallId) {
                  ...
                  try {
                    ...
                    logger.warn(
                      'Failed to mark tool complete on accept; sending fallback mark-complete',
                      e as any
                    )
                    try {
        ...
        rejectChanges: async () => {
          ...
          try {
            const { getCopilotStore } = await import('@/stores/copilot/store')
            const { currentChat, messages, toolCallsById } = getCopilotStore().getState() as any
            ...
            if (toolCallId) {
              ...
              try {
                ...
                logger.warn(
                  'Failed to mark tool complete on reject; sending fallback mark-complete',
                  e as any
                )
                try {
                  await fetch('/api/copilot/tools/mark-complete', {
