...
const createWorkflowStoreState =
  (channelId: string): WorkflowStoreStateCreator =>
  (set, get) => ({
    ...initialState,
    ...
    toggleBlockLocked: (id: string) => {
      ...
      const targetLocked = !targetBlock.locked
    ...
    toggleBlockTriggerMode: (id: string) => {
      ...
      const handleWebhookToggle = async () => {
        try {
          ...
          const response = await fetch(`/api/webhooks?workflowId=${activeWorkflowId}&blockId=${id}`)
          if (response.ok) {
...
const getStoreForWorkflow = (channelKey: string, workflowId: string) => {
  ...
  if (!workflowStoreByWorkflowId.has(bindingKey)) {
    workflowStoreByWorkflowId.set(bindingKey, createWorkflowStore(channelKey))
  }
