...
describe('workflow registry linked color-pair projection', () => {
  it('projects Yjs colorPairs workflow links into pair channels one-way', () => {
    useWorkflowRegistry.getState().syncLinkedWorkflowChannelsFromColorPairs({
      pairs: [
        { color: 'red', workflowId: 'wf-red' },
        { color: 'blue', workflowId: 'wf-blue', listing: null },
      ],
    })

    const state = useWorkflowRegistry.getState()
    expect(state.activeWorkflowIds['pair-red']).toBe('wf-red')
    expect(state.loadedWorkflowIds['pair-red']).toBe(true)
    expect(state.hydrationByChannel['pair-red']?.workflowId).toBe('wf-red')
    expect(state.activeWorkflowIds['pair-blue']).toBe('wf-blue')
  })

  it('removes pair channels whose color pair lost its workflow link', () => {
    useWorkflowRegistry.getState().syncLinkedWorkflowChannelsFromColorPairs({
      pairs: [{ color: 'red', workflowId: 'wf-red' }],
    })
    useWorkflowRegistry.getState().syncLinkedWorkflowChannelsFromColorPairs({
      pairs: [{ color: 'red', listing: null }],
    })

    const state = useWorkflowRegistry.getState()
    expect(state.activeWorkflowIds['pair-red']).toBeUndefined()
    expect(state.loadedWorkflowIds['pair-red']).toBeUndefined()
    expect(state.hydrationByChannel['pair-red']).toBeUndefined()
  })

  it('drops stale pair channels missing from the projected colorPairs entirely', () => {
    useWorkflowRegistry.getState().syncLinkedWorkflowChannelsFromColorPairs({
      pairs: [
        { color: 'red', workflowId: 'wf-red' },
        { color: 'green', workflowId: 'wf-green' },
      ],
    })
    useWorkflowRegistry.getState().syncLinkedWorkflowChannelsFromColorPairs({
      pairs: [{ color: 'green', workflowId: 'wf-green' }],
    })

    const state = useWorkflowRegistry.getState()
    expect(state.activeWorkflowIds['pair-red']).toBeUndefined()
    expect(state.activeWorkflowIds['pair-green']).toBe('wf-green')
  })

  it('preserves existing hydration state when the linked workflow is unchanged', () => {
    useWorkflowRegistry.getState().syncLinkedWorkflowChannelsFromColorPairs({
      pairs: [{ color: 'red', workflowId: 'wf-red' }],
    })
    const firstHydration =
      useWorkflowRegistry.getState().hydrationByChannel['pair-red']
