...
export function DateTimePicker({
  ...
  const dislayFormat = useMemo(() => {
    ...
    if (hideTime) {
      return format(displayValue, 'MMMM d, yyyy')
    }

    const timeParts: string[] = []
    const showHour = timePicker?.hour !== false
    const showMinute = timePicker?.minute !== false
    const showSecond = timePicker?.second !== false

    if (showHour) {
      timeParts.push(use12HourFormat ? 'hh' : 'HH')
    }
    if (showMinute) {
      timeParts.push('mm')
    }
    ...
    if (timeParts.length === 0) {
      timeParts.push(use12HourFormat ? 'hh' : 'HH', 'mm')
    }

    const timeFormat = `${timeParts.join(':')}${use12HourFormat ? ' a' : ''}`
    return format(displayValue, `MMM d, yyyy ${timeFormat}`)
  }, [displayValue, hideTime, use12HourFormat, timePicker, placeholder])
        ...
        <PopoverTrigger
          ...
          render={
            <Button
              ...
              disabled={disabled}
              className={cn(
                'h-9 w-full justify-start px-3 font-normal shadow-sm',
        ...
        <PopoverContent className='w-auto p-2'>
          <div className='flex items-center justify-between'>
            <div className='ms-2 flex cursor-pointer items-center font-bold text-md'>
              <div>
                <button
                  type='button'
                  aria-expanded={monthYearPicker === 'month'}
                ...
                <button
                  type='button'
                  aria-expanded={monthYearPicker === 'year'}
                  className='ms-1 rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2'
              ...
              <Button
                ...
                variant='ghost'
                size='icon'
                aria-label={monthYearPicker ? labels.showCalendar : labels.openMonthYearPicker}
                aria-expanded={Boolean(monthYearPicker)}
          ...
          <div className='flex flex-col gap-2'>
            ...
            <div className='flex flex-row-reverse items-center justify-between'>
              <Button className='ms-2 h-7 px-2' onClick={handleDone}>
                Done
              </Button>
              {timezone && (
