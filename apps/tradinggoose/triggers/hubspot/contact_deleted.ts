...
import {
  buildContactDeletedOutputs,
  hubspotContactTriggerOptions,
  hubspotSetupInstructions,
} from '@/triggers/hubspot/utils'
import type { TriggerConfig } from '@/triggers/types'

export const hubspotContactDeletedTrigger: TriggerConfig = {
  id: 'hubspot_contact_deleted',
  ...
  subBlocks: [
    ...
    },
    {
      ...
      defaultValue: hubspotSetupInstructions(
        'contact.deletion',
        'The webhook will trigger whenever a contact is deleted from your HubSpot account. Note: This does not include privacy deletions (GDPR compliance), which have a separate trigger.'
      ),
      mode: 'trigger',
      condition: {
        field: 'selectedTriggerId',
  ...
  -d '{
    ...
    "active": true
      ...
      defaultCollapsed: true,
      showCopyButton: true,
      description: 'Run this command to subscribe to contact deletion events',
      mode: 'trigger',
      condition: {
        field: 'selectedTriggerId',
        value: 'hubspot_contact_deleted',
  ...
  outputs: buildContactDeletedOutputs(),
