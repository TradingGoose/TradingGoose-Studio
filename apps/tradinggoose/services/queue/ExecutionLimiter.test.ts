...
describe('ExecutionLimiter', () => {
  ...
  describe('checkRateLimitWithSubscription', () => {
    ...
    it('atomically resets the requested window at the exact boundary', async () => {
      vi.useFakeTimers()
      vi.setSystemTime(new Date('2026-01-01T00:10:00.000Z'))
      const existingWindowStart = new Date('2026-01-01T00:00:00.000Z')
      vi.mocked(db.select).mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockReturnValue({
          ...
          returning: vi.fn().mockResolvedValue([
            {
              ...
              windowStart: new Date('2026-01-01T00:10:00.000Z'),
            },
          ]),
        }),
      })
      ...
      expect(result.resetAt).toEqual(new Date('2026-01-01T00:20:00.000Z'))
    ...
    it('increments a concurrently advanced window in the atomic conflict clause', async () => {
      ...
      expect(result).toMatchObject({ allowed: true, remaining: 8 })
      expect(result.resetAt).toEqual(new Date('2026-01-01T00:20:00.000Z'))
    ...
    it('uses the requested window for fail-closed dependency resets', async () => {
      ...
      const result = await rateLimiter.checkRateLimitWithSubscription(
        ...
        false,
        null,
        { failClosedOnError: true, windowMs: 600_000 }
      )
