...
describe('Trading widget order route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('rejects invalid JSON before auth or broker calls', async () => {
    const { POST } = await import('@/app/api/widgets/trading/order/route')
    const response = await POST(
      new Request('http://localhost:3000/api/widgets/trading/order', {
        method: 'POST',
        body: '{',
      })
    )
