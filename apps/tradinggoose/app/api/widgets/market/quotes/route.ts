...
export async function POST(request: NextRequest) {
  try {
    ...
    const credentialValidation = validateMarketProviderCredentialRefs(
      parsed.provider,
      parsed.auth,
    ...
    if (!credentialValidation.valid) {
      return NextResponse.json(
        {
          error: 'Market provider credentials must use environment variable references',
          details: { fields: credentialValidation.fields },
        },
        { status: 400 }
      )
    }
