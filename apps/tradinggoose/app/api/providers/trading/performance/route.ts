...
import {
  ...
  resolveTradingProviderPerformanceContext,
...
export async function POST(request: Request) {
  const requestId = createTradingProviderRequestId('performance')
  ...
  try {
    const context = await resolveTradingProviderPerformanceContext({
      requestData,
      requestId,
    })
