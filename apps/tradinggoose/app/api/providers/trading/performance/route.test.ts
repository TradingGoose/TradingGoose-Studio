...
describe('Trading provider performance route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('rejects unsupported windows', async () => {
    const { POST } = await import('@/app/api/providers/trading/performance/route')
  ...
  it('rejects invalid JSON payloads before shared preflight runs', async () => {
    const { POST } = await import('@/app/api/providers/trading/performance/route')
    const request = new NextRequest('http://localhost/api/providers/trading/performance', {
      method: 'POST',
