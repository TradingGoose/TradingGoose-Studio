...
describe('Trading provider snapshot route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('rejects provider mismatches before snapshot resolution', async () => {
    ...
    const { POST } = await import('@/app/api/providers/trading/snapshot/route')
  ...
  it('rejects invalid JSON payloads before shared preflight runs', async () => {
    const { POST } = await import('@/app/api/providers/trading/snapshot/route')
    const request = new NextRequest('http://localhost/api/providers/trading/snapshot', {
      method: 'POST',
