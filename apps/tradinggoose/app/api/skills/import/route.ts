...
const SkillsImportRequestSchema = z.object({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!authResult.success || !authResult.userId) {
      logger.warn(`[${requestId}] Unauthorized skills import attempt`)
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }

    const body = await request.json()
    const { workspaceId, file } = SkillsImportRequestSchema.parse(body)
    ...
    const parsedSkills = parseImportedSkillsFile(file)
    ...
    logger.error(`[${requestId}] Error importing skills`, { error })
