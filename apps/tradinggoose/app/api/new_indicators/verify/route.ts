...
import { mapMarketSeriesToBarsMs } from '@/lib/new_indicators/series-data'

export const runtime = 'nodejs'
export const dynamic = 'force-dynamic'
export const maxDuration = 30

const logger = createLogger('PineIndicatorVerifyAPI')
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!authResult.success || !authResult.userId) {
      logger.warn(`[${requestId}] Unauthorized pine indicator verify attempt`)
    ...
    if (!compiled.output) {
      ...
      logger.warn(`[${requestId}] Pine indicator verify failed`, {
    ...
    logger.error(`[${requestId}] Pine indicator verify failed`, { error })
    return NextResponse.json(
