...
import { compilePineIndicator } from '@/lib/new_indicators/custom/compile'
import { DEFAULT_PINE_INDICATOR_MAP } from '@/lib/new_indicators/default'
import { buildInputsMapFromMeta, normalizeInputMetaMap } from '@/lib/new_indicators/input-meta'
...
const logger = createLogger('PineIndicatorExecuteAPI')
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!authResult.success || !authResult.userId) {
      logger.warn(`[${requestId}] Unauthorized pine indicator execute attempt`)
    ...
    const customIndicatorIds = indicatorIds.filter((id) => !DEFAULT_PINE_INDICATOR_MAP.has(id))
    ...
    for (const indicatorId of indicatorIds) {
      ...
      const defaultIndicator = DEFAULT_PINE_INDICATOR_MAP.get(indicatorId)
      ...
      const compilePromise = compilePineIndicator({
    ...
    logger.error(`[${requestId}] Pine indicator execute failed`, { error })
    return NextResponse.json(
