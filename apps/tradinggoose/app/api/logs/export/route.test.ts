...
describe('logs export route', () => {
  ...
  it('exports only logs that match the requested folder name', async () => {
    const { GET } = await import('./route')
    const response = await GET(
      new NextRequest('http://localhost/api/logs/export?workspaceId=workspace-1&folderName=Alpha')
    )
  ...
  it('uses stored workflow summary fields for deleted workflow export filters', async () => {
    ...
    const response = await GET(
      new NextRequest(
        'http://localhost/api/logs/export?workspaceId=workspace-1&workflowIds=deleted-workflow-1&folderIds=folder-1'
      )
    )
