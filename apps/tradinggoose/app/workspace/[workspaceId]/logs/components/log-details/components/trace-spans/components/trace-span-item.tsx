...
export function TraceSpanItem({
  ...
  const getBlockIconAndColor = () => {
    ...
    const resolveFromBlock = (block: ReturnType<typeof getBlock> | null | undefined) => {
    ...
    if (type === 'tool') {
      ...
      for (let i = parts.length; i > 0; i--) {
        ...
        const resolved = resolveFromBlock(block)
        if (resolved) return resolved
      }
    }

    const blockType = type === 'model' ? 'agent' : type
    const resolved = resolveFromBlock(getBlock(blockType))
    if (resolved) return resolved

    if (type === 'loop' || type === 'loop-iteration') {
      return { Icon: RepeatIcon, bgColor: '#00ccff' }
    }
    if (type === 'parallel' || type === 'parallel-iteration') {
      return { Icon: SplitIcon, bgColor: '#ffdd00' }
    }

    return { Icon: Code, bgColor: '#6b7280' }
  }
  ...
  const { Icon: BlockIcon, bgColor: blockIconColor } = getBlockIconAndColor()
