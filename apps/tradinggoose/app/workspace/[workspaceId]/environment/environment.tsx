...
export function WorkspaceEnvironmentPage() {
  ...
  const scopeLabel =
    keyScope === 'workspace' ? environmentCopy.scope.workspace : environmentCopy.scope.personal

  const headerLeft = (
    <div className='flex w-full flex-1 items-center gap-3'>
      <div className='hidden items-center gap-2 sm:flex'>
        <Braces className='h-[18px] w-[18px] text-muted-foreground' />
        <span className='font-medium text-sm'>{environmentCopy.title}</span>
      </div>
      <div className='flex w-full max-w-xl flex-1'>
        <div className='flex h-9 w-full items-center gap-2 rounded-lg border bg-background pr-2 pl-3'>
          ...
          <Input
            placeholder={environmentCopy.searchPlaceholder}
            value={searchTerm}
  ...
  const headerCenter = (
    <div className='inline-flex h-9 items-center gap-1 rounded-md border bg-muted p-1 shadow-sm'>
      ...
      >
        {environmentCopy.scope.workspace}
      </Button>
      <Button
        variant='ghost'
        size='sm'
      ...
      >
        {environmentCopy.scope.personal}
      </Button>
    </div>
  )
