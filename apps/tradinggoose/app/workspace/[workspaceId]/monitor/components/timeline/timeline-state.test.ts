...
describe('buildMonitorTimelineGroups', () => {
  it('falls back to startedAt when the source log has no endedAt', () => {
    const groups = buildMonitorTimelineGroups(
      [buildExecution({ endedAt: null })],
      DEFAULT_EXECUTION_MONITOR_VIEW_CONFIG
    )

    expect(groups[0]?.items[0]?.startAt.toISOString()).toBe('2026-04-23T00:00:00.000Z')
    expect(groups[0]?.items[0]?.endAt.toISOString()).toBe('2026-04-23T00:00:00.000Z')
  })
  ...
  it('uses the shared execution ordering helper for group ordering', () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(['Running', 'Error', 'Success'])
  })
