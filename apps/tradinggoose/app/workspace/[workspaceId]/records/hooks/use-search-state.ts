...
const mergeSegments = (current: QuerySegment[], next: QuerySegment[]) => {
  const nextClauseRaws = new Set(
    next
      .filter(
        (segment): segment is Extract<QuerySegment, { kind: 'clause' }> => segment.kind === 'clause'
      )
      .map((segment) => segment.clause.raw)
  )

  const preservedCurrent = current.filter(
    (segment) => segment.kind !== 'clause' || !nextClauseRaws.has(segment.clause.raw)
  )
...
const getClausesFromSegments = (segments: QuerySegment[]) =>
  segments.flatMap((segment) => (segment.kind === 'clause' ? [segment.clause] : []))
...
export function useSearchState({
  ...
  const [invalidQualifierFragments, setInvalidQualifierFragments] = useState<string[]>([])
  ...
  const handleSuggestionSelect = useCallback(
    (suggestion: Suggestion) => {
      ...
      if (suggestion.category === 'qualifier' && suggestion.value.endsWith(':')) {
