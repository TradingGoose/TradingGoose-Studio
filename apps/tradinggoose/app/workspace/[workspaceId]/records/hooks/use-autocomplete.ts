...
import type { Suggestion, SuggestionGroup } from '@/lib/logs/search-suggestion-types'
...
interface AutocompleteState {
  ...
  suggestionType: SuggestionGroup['type'] | null
...
export function useAutocomplete({
  ...
  const handleSuggestionSelect = useCallback(
    (suggestion?: Suggestion) => {
      ...
      if (state.suggestionType === 'qualifiers' && selectedSuggestion.value.endsWith(':')) {
        newCursorPosition = newValue.lastIndexOf(':') + 1
      } else if (state.suggestionType === 'values') {
        newValue = `${newValue} `
