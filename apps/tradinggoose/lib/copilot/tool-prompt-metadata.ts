...
export const TOOL_PROMPT_METADATA: Record<ToolId, ToolPromptMetadata> = {
  ...
  edit_workflow: {
    description:
      'Rewrite the full workflow graph topology using exact argument keys `entityId` and minimal Mermaid `entityDocument`, then return the resulting workflow state and canonical `tg-mermaid-v1` workflow document. Use this only for graph or topology edits such as adding, deleting, reconnecting blocks, or changing loop/parallel nesting. Do not send `documentFormat`, `TG_BLOCK`, `TG_EDGE`, `subBlocks`, condition branch labels, `outputs`, `enabled`, positions, or full block metadata. Existing block ids are stable identities: their type and details are preserved by exact id. This tool cannot replace an existing block or change its type; new ids create new blocks with generated positions. New blocks need `id:` and canonical `type:` labels. Existing condition edges must use exact `condition-<blockId>-<branch>` source handles; use `edit_workflow_block` to define branches. If any existing block id is intentionally deleted, include it in `removedBlockIds`; otherwise every existing block id must remain in the Mermaid graph. Use `edit_workflow_block` for one existing block `name`, `enabled`, `subBlocks`, or condition branch definition change.',
    kind: 'edit',
    entityKind: 'workflow',
  },
  edit_workflow_block: {
