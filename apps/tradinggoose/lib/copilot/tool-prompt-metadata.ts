...
export const TOOL_PROMPT_METADATA: Record<ToolId, ToolPromptMetadata> = {
  ...
  edit_workflow: {
    description:
      'Replace the full workflow graph using exact argument keys `entityId` and minimal Mermaid `entityDocument`, then return the resulting workflow state and graph-only Mermaid document. Use this only for graph or topology edits such as adding, removing, reconnecting, replacing blocks, or changing loop/parallel nesting. Do not send `documentFormat`, `TG_BLOCK`, `TG_EDGE`, `subBlocks`, condition branch labels, `outputs`, `enabled`, positions, or full block metadata. Existing block details are preserved by exact block id. New blocks need `id:` and canonical `type:` labels. Existing condition edges must use exact `condition-<blockId>-<branch>` source handles; use `edit_workflow_block` to define branches. If any existing block id is intentionally removed or replaced, include it in `removedBlockIds`; otherwise every existing block id must remain in the Mermaid graph. Use `edit_workflow_block` for one existing block `name`, `enabled`, `subBlocks`, or condition branch definition change.',
    kind: 'edit',
    entityKind: 'workflow',
  },
  edit_workflow_block: {
