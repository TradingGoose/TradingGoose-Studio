...
export const ContextUsagePill = memo(({ percentage, onCreateNewChat }: ContextUsagePillProps) => {
    ...
    >
      ...
      {isHighUsage && (
        <Tooltip>
          ...
          />
          <TooltipContent side='top'>{copilotCopy.contextUsage.recommendedNewChat}</TooltipContent>
        </Tooltip>
      )}
    </div>
  )
