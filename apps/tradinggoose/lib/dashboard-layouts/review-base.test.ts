...
describe('dashboard review bases', () => {
  it('hashes only topology for a layout edit, never child widget documents', () => {
    ...
    const hash = (current: DashboardLayoutProjectionContent) =>
      hashServerToolReviewBase(buildDashboardLayoutReviewBase(current))
  ...
  it('tracks only color-pair fields overwritten by a widget patch', () => {
    const content = createContent()
    ...
    destinationChanged.colorPairs.pairs[2].listing = listing('GOOG')
    const unrelatedFieldChanged = structuredClone(content)
    unrelatedFieldChanged.colorPairs.pairs[2].workflowId = 'workflow-2'
