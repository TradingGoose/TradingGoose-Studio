...
describe('runPreparedWorkflowExecution', () => {
  ...
  it('rejects executor output completed after the tier time limit', async () => {
    ...
    mocks.execute.mockImplementationOnce(async () => {
      ...
      return { success: true, output: { tooLate: true }, logs: [] }
    })
    ...
    try {
      const result = await runPreparedWorkflowExecution({
        ...
        triggerType: 'manual',
        workflowInput: {},
        executionId: 'execution-time-limit',
        triggerTarget: { kind: 'trigger', triggerType: 'manual' },
      })
      ...
      expect(mocks.executorConstructor).toHaveBeenCalledWith(
        expect.objectContaining({
          contextExtensions: expect.objectContaining({ abortSignal: expect.any(AbortSignal) }),
        })
      )
    } finally {
      now.mockRestore()
  ...
  it('aborts an in-flight executor when the tier time limit elapses', async () => {
    vi.useFakeTimers()
    let markExecutorReady!: () => void
    const executorReady = new Promise<void>((resolve) => {
      markExecutorReady = resolve
    })
    mocks.checkServerSideUsageLimits.mockResolvedValueOnce({
      isExceeded: false,
      currentUsage: 0,
    ...
    mocks.execute.mockImplementationOnce(
      () =>
        new Promise((_resolve, reject) => {
          const signal = mocks.executorConstructor.mock.calls.at(-1)?.[0].contextExtensions
