...
import type { PineIndicatorOptions } from '@/lib/new_indicators/types'
...
const INDICATOR_DEFAULTS: Required<PineIndicatorOptions> = {
  overlay: false,
  format: 'inherit',
  precision: 10,
  scale: 'points',
...
const normalizeOptionValue = (
  key: keyof PineIndicatorOptions,
  value: unknown
): PineIndicatorOptions[keyof PineIndicatorOptions] | undefined => {
...
): PineIndicatorOptions | undefined => {
  if (!raw || typeof raw !== 'object') return undefined
  const result: PineIndicatorOptions = {}
  const dropDefaults = options?.dropDefaults ?? false

  INDICATOR_OPTION_KEYS.forEach((key) => {
    const value = normalizeOptionValue(
      key as keyof PineIndicatorOptions,
      (raw as Record<string, unknown>)[key]
    )
    if (typeof value === 'undefined') return
    if (dropDefaults) {
      const defaultValue = INDICATOR_DEFAULTS[key as keyof PineIndicatorOptions]
...
const parseIndicatorOptionsFromObject = (
  ...
  node.properties.forEach((prop) => {
    ...
    if (typeof value === 'undefined') return
    const normalized = normalizeOptionValue(key as keyof PineIndicatorOptions, value)
...
export const inferIndicatorOptionsFromPineCode = (
  ...
  let inferred: PineIndicatorOptions | undefined
