...
export function WorkspaceInviteModal({
  ...
  const optionalRoute = useOptionalWorkflowRoute()
  ...
  const currentUserEmailFromPermissions = workspacePermissions?.users?.find(
    (user) => user.userId === currentUserId
  )?.email
  const normalizedCurrentUserEmail =
    currentUserEmailFromPermissions?.trim().toLowerCase() ??
          ...
          data.invitations
            ?.filter(
              (inv: PendingInvitation) =>
                inv.status === 'pending' && inv.workspaceId === resolvedWorkspaceId
            )
  ...
  const handleSaveChanges = useCallback(async () => {
    if (!userPerms.canAdmin || !hasPendingChanges || !resolvedWorkspaceId) return
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.WORKSPACE_PERMISSIONS(resolvedWorkspaceId), {
        method: 'PATCH',
        headers: {
  ...
  const handleRemoveMemberConfirm = useCallback(async () => {
    if (!memberToRemove || !resolvedWorkspaceId || !userPerms.canAdmin) return
  ...
  const handleRemoveInvitationConfirm = useCallback(async () => {
    if (!invitationToRemove || !resolvedWorkspaceId || !userPerms.canAdmin) return
  ...
  }, [invitationToRemove, resolvedWorkspaceId, userPerms.canAdmin])

  const handleResendInvitation = useCallback(
    async (invitationId: string, email: string) => {
      if (!resolvedWorkspaceId || !userPerms.canAdmin) return
    ...
    [resolvedWorkspaceId, userPerms.canAdmin, resendCooldowns]
  )
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (emails.length === 0 || !resolvedWorkspaceId) {
        return
      }
      ...
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter className='flex'>
            <AlertDialogCancel
              className='h-9 w-full rounded-sm'
              onClick={() => setInvitationToRemove(null)}
  ...
  return (
    <>
      {inviteWorkspace && workspaceUser ? (
        <WorkspacePermissionsProvider workspaceId={inviteWorkspace.id} userId={workspaceUser.id}>
          <WorkspaceInviteModal
            open={inviteDialogOpen}
            onOpenChange={onInviteDialogChange}
            currentUserId={workspaceUser.id}
            currentUserEmail={workspaceUser.email}
