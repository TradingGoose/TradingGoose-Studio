...
const BLOCKED_TIME_SCALE_OVERRIDE_KEYS = [
  'enableConflation',
...
export const sanitizeStyleOverrides = (
  ...
  entries.forEach(([key, value]) => {
    if (!ALLOWED_STYLE_OVERRIDE_KEYS.has(key)) {
      onWarning?.(`data_chart ignored unsupported style override: ${key}`)
      return
    }
    sanitized[key] = value
  })
  ...
  if (sanitized.timeScale && typeof sanitized.timeScale === 'object') {
    ...
    BLOCKED_TIME_SCALE_OVERRIDE_KEYS.forEach((key) => {
      if (key in timeScale) {
        onWarning?.(`data_chart ignores timeScale.${key} overrides`)
        delete timeScale[key]
      }
    })
