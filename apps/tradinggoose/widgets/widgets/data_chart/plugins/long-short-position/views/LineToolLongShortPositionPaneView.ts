...
import type { LineToolLongShortPosition } from '../model/LineToolLongShortPosition'
...
> extends LineToolPaneView<HorzScaleItem> {
  ...
  protected override _updateImpl(height: number, width: number): void {
    ...
    if (options.showAutoText) {
      ...
      const finalRiskTextOptions = merge(
      ...
      finalRiskTextOptions.value =
        riskUserNote && riskUserNote.trim().length > 0 ? `${riskStats}\n${riskUserNote}` : riskStats
      ...
      if (finalRiskTextOptions.box.alignment.vertical === BoxVerticalAlignment.Middle) {
        finalRiskTextOptions.box.alignment.vertical = isLong
      ...
      if (P_PT_Screen && P2_logical) {
        ...
        // A. Merge Defaults + User Options
        const finalRewardTextOptions = merge(
          deepCopy(defaultAutoTextStyle),
          options.entryPtText
        ) as TextOptions
        ...
        // Append user note on a new line if it exists
        finalRewardTextOptions.value =
          rewardUserNote && rewardUserNote.trim().length > 0
            ? `${rewardStats}\n${rewardUserNote}`
            : rewardStats
        ...
        if (finalRewardTextOptions.box.alignment.vertical === BoxVerticalAlignment.Middle) {
          finalRewardTextOptions.box.alignment.vertical = isLong
            ? BoxVerticalAlignment.Top
            : BoxVerticalAlignment.Bottom
        }
