...
export class LineToolRectanglePaneView<HorzScaleItem> extends LineToolPaneView<HorzScaleItem> {
  ...
  protected override _addAnchors(renderer: CompositeRenderer<any>): void {
    ...
    const diag1Cursor =
      sign < 0 ? PaneCursorType.DiagonalNeSwResize : PaneCursorType.DiagonalNwSeResize // NE-SW or NW-SE
    const diag2Cursor =
      sign < 0 ? PaneCursorType.DiagonalNwSeResize : PaneCursorType.DiagonalNeSwResize // NW-SE or NE-SW
    ...
    // 6: Top-Center (TC)
    const topCenter = new AnchorPoint(
    ...
    // 5: Middle-Right (MR)
    const middleRight = new AnchorPoint(
      maxX,
      ((minY + maxY) / 2) as Coordinate,
      5,
      true,
    ...
    // 7: Bottom-Center (BC)
    const bottomCenter = new AnchorPoint(
      ((minX + maxX) / 2) as Coordinate,
      maxY,
      7,
      true,
    ...
    // 4: Middle-Left (ML)
    const middleLeft = new AnchorPoint(
      minX,
      ((minY + maxY) / 2) as Coordinate,
      4,
      true,
