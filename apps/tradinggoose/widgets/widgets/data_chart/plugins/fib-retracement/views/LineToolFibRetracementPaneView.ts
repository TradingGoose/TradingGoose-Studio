...
import type { LineToolFibRetracement } from '../model/LineToolFibRetracement'
...
export class LineToolFibRetracementPaneView<HorzScaleItem> extends LineToolPaneView<HorzScaleItem> {
   ...
   * @protected
   */
  protected _primaryLineRenderer: SegmentRenderer<HorzScaleItem> = new SegmentRenderer(
    new HitTestResult(HitTestType.MovePoint)
  )
  ...
  private _getFadedColor(color: string, opacity: number): string {
    ...
    if (color.startsWith('#')) {
      ...
      if (matches && matches.length >= 3) {
        r = Number.parseFloat(matches[0])
        g = Number.parseFloat(matches[1])
        b = Number.parseFloat(matches[2])
      } else {
  ...
  protected override _updateImpl(height: number, width: number): void {
    ...
    for (let i = 0; i < levelsConfig.length; i++) {
      ...
      // --- A. Text Label Setup ---
      const distanceText = this._calculateDistanceText(
        config,
        levelPrice,
        levelsConfig,
        allDerivedLevelCoordinates
      )
