...
import {
  ...
  type MarketDepthAggregatesData,
  // Types for Data/Options
  type MarketDepthSingleAggregatesData,
...
export class MarketDepthRenderer<HorzScaleItem> implements IPaneRenderer {
  ...
  public draw(target: CanvasRenderingTarget2D): void {
    ...
    target.useMediaCoordinateSpace(
      ({ context: ctx, mediaSize }: MediaCoordinatesRenderingScope) => {
         ...
         */
        const highestSizes = this._calculateHighestTotalSize(
          Bids,
          Asks,
          options.totalBidAskCalcMethod
        )
        ...
        const bidMaxScale =
          options.totalBidAskCalcMethod === 'combined'
            ? highestCombinedTotalSize
            : highestBidTotalSize
   ...
   * @private
   */
  private _calculateHighestTotalSize(
    Bids: MarketDepthSingleAggregatesData[],
    Asks: MarketDepthSingleAggregatesData[],
    calcMethod: string
  ): { highestBid: number; highestAsk: number; highestCombined: number } {
    const bidSizes = Bids.map((b) => Number.parseFloat(b.TotalSize || '0'))
    const askSizes = Asks.map((a) => Number.parseFloat(a.TotalSize || '0'))
