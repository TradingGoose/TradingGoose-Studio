...
export class ParallelChannelRenderer<HorzScaleItem> implements IPaneRenderer {
   ...
   */
  public constructor() {
    this._hitTestLine = new HitTestResult(HitTestType.MovePoint, {
      pointIndex: null,
      suggestedCursor: PaneCursorType.Pointer,
    })
    this._hitTestBackground = new HitTestResult(HitTestType.MovePointBackground, {
      pointIndex: null,
  ...
  public draw(target: CanvasRenderingTarget2D): void {
    ...
    target.useMediaCoordinateSpace(
      ({ context: ctx, mediaSize }: MediaCoordinatesRenderingScope) => {
        ...
        if (lineTop.b !== 0) {
          if (extend.left) {
            p_start_top = new Point(
              X_FAR_LEFT as Coordinate,
              (m_top * X_FAR_LEFT + b_top) as Coordinate
            )
            p_start_bottom = new Point(
              X_FAR_LEFT as Coordinate,
              (m_bottom * X_FAR_LEFT + b_bottom) as Coordinate
            )
          }

          if (extend.right) {
            p_end_top = new Point(
              X_FAR_RIGHT as Coordinate,
              (m_top * X_FAR_RIGHT + b_top) as Coordinate
            )
            p_end_bottom = new Point(
              X_FAR_RIGHT as Coordinate,
              (m_bottom * X_FAR_RIGHT + b_bottom) as Coordinate
            )
          }
        }
