...
import { type AnchorPoint, LineAnchorRenderer } from '../rendering/line-anchor-renderer'
import {
  HitTestType,
  type IPaneRenderer,
  type IUpdatablePaneView,
  type LineAnchorCreationData,
...
export abstract class LineToolPaneView<HorzScaleItem> implements IUpdatablePaneView {
   ...
   * @protected
   */
  protected _invalidated = true // Flag to indicate if the view needs updating
   ...
   * @protected
   */
  protected createLineAnchor(
    data: LineAnchorCreationData,
    index: number
  ): LineAnchorRenderer<HorzScaleItem> {
  ...
  protected _lineAnchorColors(points: AnchorPoint[]): string[] {
    ...
    const defaultAnchorColor =
      backgroundColor.type === 'solid' ? backgroundColor.color : 'transparent'
