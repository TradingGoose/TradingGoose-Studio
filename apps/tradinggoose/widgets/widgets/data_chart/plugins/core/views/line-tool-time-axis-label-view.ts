...
import type { Coordinate, IChartApiBase, ITimeScaleApi, Logical } from 'lightweight-charts'
...
export class LineToolTimeAxisLabelView<HorzScaleItem> implements ITimeAxisView {
  ...
  private _updateImpl(): void {
    ...
    const isToolActive =
      this._tool.isSelected() ||
      this._tool.isHovered() ||
      this._tool.isEditing() ||
      this._tool.isCreating()
    ...
    if (
      !toolOptions.visible ||
      !toolOptions.showTimeAxisLabels ||
      !(toolOptions.timeAxisLabelAlwaysVisible || isToolActive)
    ) {
      return
    }
    ...
    const internalHorzItemForFormatting =
      this._tool.horzScaleBehavior.convertHorzItemToInternal(timeAsHorzScaleItem)
    ...
    // Convert the timestamp to a logical index with the same snap/extrapolation
    // policy used by pane rendering, so labels stay aligned with anchors.
    const interpolatedLogicalIndex = this._tool.logicalIndexForTimestamp(point.timestamp, true)

    if (interpolatedLogicalIndex === null) {
      console.warn(
        `[TimeLabelView] Skipping update: Time-to-Index Interpolation failed for timestamp ${point.timestamp}`
      )
      return
    }

    // Convert the interpolated Logical Index into a Screen Coordinate.
    const coordinate = this._timeScale.logicalToCoordinate(
