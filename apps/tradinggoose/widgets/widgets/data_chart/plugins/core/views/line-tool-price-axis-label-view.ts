...
import type { Coordinate, IChartApiBase, ISeriesPrimitiveAxisView } from 'lightweight-charts'
import type { BaseLineTool } from '../model/base-line-tool'
import type { PriceAxisLabelStackingManager } from '../model/price-axis-label-stacking-manager'
 ...
 */
export class LineToolPriceAxisLabelView<HorzScaleItem>
  extends PriceAxisView
  ...
  protected _updateRendererData(
    ...
    const currentRendererOptions: PriceAxisViewRendererOptions = {
      ...
      paddingInner: Math.max(
        Math.ceil(layoutOptions.fontSize / 2 - (priceScaleOptions.ticksVisible ? 4 : 0) / 2),
        0
      ),
      paddingOuter: Math.ceil(
        layoutOptions.fontSize / 2 + (priceScaleOptions.ticksVisible ? 4 : 0) / 2
      ),
    ...
    try {
      ...
      const tempRendererData: PriceAxisViewRendererData = {
        text: textToMeasure,
        visible: true,
        tickVisible: false,
      } as any
      const tempCommonData: PriceAxisViewRendererCommonData = {
    ...
    this._priceAxisLabelStackingManager.registerLabel({
      ...
      height: labelHeight,
      setFixedCoordinate: (coord: Coordinate | undefined) =>
        this.setFixedCoordinateFromManager(coord),
