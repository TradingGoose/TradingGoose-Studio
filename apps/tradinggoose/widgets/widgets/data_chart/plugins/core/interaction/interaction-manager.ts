...
export class InteractionManager<HorzScaleItem> {
  ...
  private readonly _boundHandleMouseDown = (event: MouseEvent): void => {
    if (this._isCompatibilityMouseEvent(this._lastPointerDownTs)) {
      return
    }
    this._handleMouseDown(event)
  }
  private readonly _boundHandleMouseMove = (event: MouseEvent): void => {
    if (this._isCompatibilityMouseEvent(this._lastPointerMoveTs)) {
      return
    }
    this._handleMouseMove(event)
  }
  private readonly _boundHandleMouseUp = (event: MouseEvent): void => {
    if (this._isCompatibilityMouseEvent(this._lastPointerUpTs)) {
      return
    }
    this._handleMouseUp(event)
  }
  private readonly _boundHandlePointerDown = (event: PointerEvent): void => {
    if (!this._isPrimaryMousePointerEvent(event)) {
      return
    }
    this._lastPointerDownTs = performance.now()
    this._handleMouseDown(event as unknown as MouseEvent)
  }
  private readonly _boundHandlePointerMove = (event: PointerEvent): void => {
    if (!this._isPrimaryMousePointerEvent(event)) {
      return
    }
    this._lastPointerMoveTs = performance.now()
    this._handleMouseMove(event as unknown as MouseEvent)
  }
  private readonly _boundHandlePointerUp = (event: PointerEvent): void => {
    if (!this._isPrimaryMousePointerEvent(event)) {
      return
    }
    this._lastPointerUpTs = performance.now()
    this._handleMouseUp(event as unknown as MouseEvent)
  }
  private readonly _boundHandleDblClick = (params: MouseEventParams<HorzScaleItem>): void =>
    this._handleDblClick(params)
  private readonly _boundHandleCrosshairMove = (params: MouseEventParams<HorzScaleItem>): void =>
  ...
  private _subscribeToChartEvents(): void {
    ...
    chartElement.addEventListener('pointerdown', this._boundHandlePointerDown)
    chartElement.addEventListener('pointermove', this._boundHandlePointerMove)
  ...
  /**
   * Releases all listeners owned by this interaction manager.
   */
  public destroy(): void {
    ...
    const chartElement = this._chart.chartElement()
    chartElement.removeEventListener('pointerdown', this._boundHandlePointerDown)
    chartElement.removeEventListener('pointermove', this._boundHandlePointerMove)
  ...
  private _isPrimaryMousePointerEvent(event: PointerEvent): boolean {
  ...
  private _isCompatibilityMouseEvent(lastPointerTs: number): boolean {
    if (lastPointerTs <= 0) {
      return false
    }
    return performance.now() - lastPointerTs < 80
  }
  ...
  private _handleMouseDown(event: MouseEvent): void {
    ...
    if (hitResult?.tool) {
      ...
      // Translation must use the canonical points, not virtual resize anchors.
      this._originalTranslationPoints = hitResult.tool.getPermanentPointsForTranslation()
      // highlight-end
  ...
  private _handleMouseMove(event: MouseEvent): void {
      ...
      if (this._isDrag && isDragCreationSupported) {
        ...
        if (this._isShiftKeyDown && isShiftConstraintSupported) {
          ...
          if (originalP0 && tool.getShiftConstrainedPoint) {
            ...
            const allOriginalLogicalPointsForCreation =
              this._originalDragPoints || (originalP0 ? [originalP0] : [])
    ...
    if (this._draggedTool && this._dragStartPoint) {
      ...
      if (this._isEditing) {
        ...
        const phase: InteractionPhase = isAnchorDrag
        ...
        if (isAnchorDrag) {
          ...
          if (!this._originalTranslationPoints || this._originalTranslationPoints.length === 0) {
            return
          }
          ...
          // 1. Get the initial logical P0 and screen P0 at drag start.
          ...
          // 2. Resolve the intended new screen position for P0.
          const newScreenP0 = initialScreenP0.add(delta)

          // 3. Resolve P0 in logical space for vertical translation.
          ...
          // 4. Resolve translation vector in logical-index space (horizontal).
          const initialLogicalIndexP0 = timeScale.coordinateToLogical(
            initialScreenP0.x as Coordinate
          )
          const newLogicalIndexP0 = timeScale.coordinateToLogical(newScreenP0.x as Coordinate)
          if (initialLogicalIndexP0 === null || newLogicalIndexP0 === null) {
            return
          }
          const logicalTranslationVector = Number(newLogicalIndexP0) - Number(initialLogicalIndexP0)
          if (!Number.isFinite(logicalTranslationVector)) {
            return
          }
          let translationSeries: ISeriesApi<SeriesType, HorzScaleItem>
          try {
            translationSeries = tool.getSeries()
          } catch {
            return
          }

          // 5. Resolve translation vector in price space (vertical).
          const priceTranslationVector = newLogicalP0.price - initialLogicalP0.price
          const newLogicalPoints: LineToolPoint[] = []

          // 6. Apply translation to canonical points.
          for (const originalLogicalPoint of this._originalTranslationPoints) {
            const originalLogicalIndex = tool.logicalIndexForTimestamp(
              originalLogicalPoint.timestamp,
              false
            )
            if (originalLogicalIndex === null) {
              return
            }
            ...
            const translatedTime = interpolateTimeFromLogicalIndex(
              this._chart,
              translationSeries,
              Number(originalLogicalIndex) + logicalTranslationVector
            )
            if (translatedTime === null) {
              return
            }

            newLogicalPoints.push({
          ...
          // 7. Update the tool with translated points.
  ...
  private _handleMouseUp(event: MouseEvent): void {
    ...
    if (
      !clickedInsideChartElement &&
      !this._isDrag &&
      !this._isCreationGesture &&
      !this._draggedTool
    ) {
    ...
    if (this._isCreationGesture && this._creationTool && this._mouseDownPoint) {
      ...
      let isDiscreteClick =
        timeDelta < CLICK_TIMEOUT && distanceMoved <= DRAG_THRESHOLD && !this._isDrag
      ...
      if (isDiscreteClick) {
        ...
        if (!supportsClickClick) {
          console.warn(
            `[InteractionManager] Tool ${tool.toolType} does not support click-click creation.`
          )
          this.setCurrentToolCreating(null)
          this.deselectAllTools()
          this._plugin.requestUpdate()
        ...
        let finalLogicalPoint: LineToolPoint | null =
          this.screenPointToLineToolPoint(finalScreenPoint)
        ...
        if (finalLogicalPoint) {
          ...
          console.warn(
            `[InteractionManager] Final logical point conversion failed. Click discarded.`
          )
        }
        ...
        if (!supportsClickDrag) {
          console.warn(
            `[InteractionManager] Tool ${tool.toolType} does not support click-drag creation.`
          )
          this.setCurrentToolCreating(null)
          this.deselectAllTools()
          this._plugin.requestUpdate()
    ...
    if (this._draggedTool && this._dragStartPoint) {
      if (this._isEditing) {
        ...
        // It was a discrete CLICK ON AN EXISTING TOOL (selection)
    ...
    const distanceMovedFinal =
      this._mouseDownPoint && point ? point.subtract(this._mouseDownPoint).length() : 0
    ...
    const wasAShortClick =
      timeDeltaFinal < CLICK_TIMEOUT && distanceMovedFinal <= DRAG_THRESHOLD && point
  ...
  private _handleCrosshairMove(params: MouseEventParams<HorzScaleItem>): void {
    ...
    if (toolBeingCreated) {
      ...
      if (
        toolBeingCreated.points().length > 0 &&
        rawScreenPoint &&
        isShiftKeyDown &&
        toolBeingCreated.supportsShiftClickClickConstraint?.() === true
      ) {
  ...
  public deselectAllTools(): void {
    // MODIFIED: Made public with a clear name
