...
import type { LineToolType } from '../types'
import type { BaseLineTool } from './base-line-tool'
...
export class ToolRegistry<HorzScaleItem> {
   ...
   * @private
   */
  private readonly _toolConstructors = new Map<
    LineToolType,
    new (
      ...args: any[]
  ...
  public getToolClass(type: LineToolType): new (...args: any[]) => BaseLineTool<HorzScaleItem> {
    ...
    if (!toolClass) {
      throw new Error(
        `Line tool type "${type}" is not registered. Ensure you have imported and registered the tool.`
      )
    }
    return toolClass
  }
