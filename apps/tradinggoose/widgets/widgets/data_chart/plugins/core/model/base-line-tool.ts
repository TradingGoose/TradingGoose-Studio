...
import type { LineToolExport, LineToolPoint } from '../api/public-api'
...
import { type DeepPartial, merge, randomHash } from '../utils/helpers'
...
import type { PriceAxisLabelStackingManager } from './price-axis-label-stacking-manager'
...
const isTimestampWithinSeriesBounds = <HorzScaleItem>(
  ...
  implements ISeriesPrimitive<HorzScaleItem>
{
  ...
  public priceAxisViews(): readonly IPriceAxisView[] {
    // Correct return type for DataSource
  ...
  protected _options: LineToolOptionsInternal<LineToolType> =
    {} as LineToolOptionsInternal<LineToolType>
  ...
  public constructor(
    ...
    if (this.pointsCount !== -1) {
      for (let i = 0; i < this.pointsCount; i++) {
        this._priceAxisLabelViews[i] = new LineToolPriceAxisLabelView(
          this,
          i,
          this._chart,
  ...
  public attached(param: SeriesAttachedParameter<HorzScaleItem>): void {
    ...
    if (!this._attachedPane) {
      console.warn(
        `[BaseLineTool] Tool ${this.id()} attached to a series not found in any pane. This primitive relies on IPaneApi access.`
      )
    }
  }
  ...
  public getPane(): IPaneApi<HorzScaleItem> {
    if (!this._attachedPane) {
      throw new Error(
        `Tool ${this.id()} is not attached to a pane. 'attached()' might not have been called or ran into an issue.`
      )
    }
  ...
  /**
   * Resolves the logical index for a timestamp with stable cross-interval behavior.
   *
   * For timestamps inside loaded series bounds, this can snap to the nearest concrete bar index.
   * For timestamps outside bounds (blank/future space), interpolation/extrapolation is preserved.
   */
  public logicalIndexForTimestamp(timestamp: number, snapToNearestBar = true): Logical | null {
    const normalizedTimestamp = toSecondsTimestamp(timestamp)
    if (normalizedTimestamp === null) {
      return null
    }

    const interpolated = interpolateLogicalIndexFromTime(
      this._chart,
      this._series,
      normalizedTimestamp as UTCTimestamp
    )
    if (interpolated === null) {
      return null
    }

    if (!snapToNearestBar || this.isEditing() || this.isCreating()) {
      return interpolated as Logical
    }

    if (!isTimestampWithinSeriesBounds(this._series, normalizedTimestamp)) {
      return interpolated as Logical
    }

    const nearestIndex = this._chart
      .timeScale()
      .timeToIndex(normalizedTimestamp as unknown as HorzScaleItem, true)
    if (nearestIndex === null || !Number.isFinite(Number(nearestIndex))) {
      return interpolated as Logical
    }

    return Number(nearestIndex) as Logical
  }
  ...
  public pointToScreenPoint(point: LineToolPoint): Point | null {
    ...
    const logicalIndex = this.logicalIndexForTimestamp(point.timestamp, true)

    if (logicalIndex === null) {
      console.warn(
        `[BaseLineTool] pointToScreenPoint: Could not determine logical index for timestamp: ${point.timestamp}.`
      )
      return null
    }
    ...
    if (x === null || y === null) {
      console.warn(
        `[BaseLineTool] pointToScreenPoint: Coordinate conversion failed for point: ${JSON.stringify(point)}. Received x=${x}, y=${y}`
      )
      return null
    }
  ...
  public screenPointToPoint(point: Point): LineToolPoint | null {
    ...
    if (interpolatedTime === null || price === null) {
      console.warn(
        `[BaseLineTool] screenPointToPoint: Could not determine interpolated time or price for screen point: ${JSON.stringify(point)}.`
      )
      return null
    }
  ...
  public _triggerChartUpdate(): void {
    if (this._requestUpdate) {
      // Use the existing _requestUpdate property
      try {
        this._requestUpdate()
      } catch {
        // Ignore disposal races during teardown.
      ...
      console.warn(
        `[BaseLineTool] Attempted to trigger chart update for tool ${this.id()} but _requestUpdate is not set.`
      )
    }
  }
