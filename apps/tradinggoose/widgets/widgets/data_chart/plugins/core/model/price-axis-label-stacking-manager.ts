...
import type { LabelDataForStacking, PriceAxisViewRendererOptions } from '../types'
...
export class PriceAxisLabelStackingManager<HorzScaleItem> {
  ...
  public updateStacking(): void {
    if (!this._priceAxisRendererOptions) {
      console.warn(
        '[PALSManager] Cannot update stacking: PriceAxisViewRendererOptions not set. Skipping stacking adjustment.'
      )
    ...
    let lastOccupiedBottomCoord: Coordinate | undefined
    ...
    for (let i = 0; i < activeLabels.length; i++) {
      ...
      const currentLabelTop = (currentLabel.originalCoordinate -
        currentLabelHalfHeight) as Coordinate
      ...
      const finalCenterY =
        newFixedCoord !== currentLabel.originalCoordinate
          ? newFixedCoord
          : currentLabel.originalCoordinate
