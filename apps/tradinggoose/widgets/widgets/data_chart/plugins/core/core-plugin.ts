...
import type { LineToolPartialOptionsMap, LineToolType } from './types'
...
export class LineToolsCorePlugin<HorzScaleItem> implements ILineToolsApi {
  ...
  private _stackingUpdateScheduled = false
  ...
  public constructor(
    ...
    this._interactionManager = new InteractionManager<HorzScaleItem>(
      this,
      this._chart,
      this._series,
      this._tools,
    ...
    this._priceAxisLabelStackingManager = new PriceAxisLabelStackingManager<HorzScaleItem>(
      this._chart,
      this._series
    )
  }
   ...
   * });
   */
  public addLineTool<T extends LineToolType>(
    type: T,
    points?: LineToolPoint[] | null,
    options?: LineToolPartialOptionsMap[T] | undefined
  ): string {
    try {
      ...
      const tool = this._createAndAddTool(
        type,
        points || [],
        options,
        undefined,
  ...
  public applyLineToolOptions<T extends LineToolType>(toolData: LineToolExport<T>): boolean {
    ...
    if (!tool || tool.toolType !== toolData.toolType) {
      console.error(
        `Cannot apply options: Tool with ID "${toolData.id}" not found or type mismatch.`
      )
      return false
    }
  ...
  public setCrossHairXY(x: number, y: number, visible: boolean): void {
    ...
    const lineToolPoint = this._interactionManager.screenPointToLineToolPoint(
      new Point(x as Coordinate, y as Coordinate)
    )
  ...
  private _createAndAddTool<T extends LineToolType>(
    ...
    options?: LineToolPartialOptionsMap[T],
    id?: string,
    initiateInteractive = false // New parameter to signal interactive drawing initiation
