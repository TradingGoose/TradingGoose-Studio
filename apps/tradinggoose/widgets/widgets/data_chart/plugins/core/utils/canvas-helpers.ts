...
export function computeDashPattern(ctx: CanvasRenderingContext2D, style: LineStyle): number[] {
  switch (style) {
    ...
    case LineStyle.Dotted:
      return [ctx.lineWidth, ctx.lineWidth]
    case LineStyle.Dashed:
      return [2 * ctx.lineWidth, 2 * ctx.lineWidth]
    case LineStyle.LargeDashed:
      return [6 * ctx.lineWidth, 6 * ctx.lineWidth]
    case LineStyle.SparseDotted:
      return [ctx.lineWidth, 4 * ctx.lineWidth]
    default:
      return []
  }
}
...
export function computeEndLineSize(lineWidth: number): number {
  switch (lineWidth) {
    ...
    case 4:
      return 1.25
    default:
      return 1 // For other widths, use 1
  }
}
...
export function getArrowPoints(point0: Point, point1: Point, width: number): Segment[] {
  ...
  if (o.length() < l * 0.1) {
    // Adjusted threshold for very short lines
    return []
  }
  ...
  return [
    ...
    [g, y],
    [T, x_val],
  ] // The three segments forming the arrow
}
...
export function drawRoundRect(
  ...
  if (Array.isArray(radius)) {
    ...
    if (radius.length === 2) {
      // Assuming [top-left/bottom-right, top-right/bottom-left] or some interpretation
