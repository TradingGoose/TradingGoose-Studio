...
export function clipPolygonToViewport(points: Point[], W: number, H: number): Point[] | null {
  ...
  clipPlanes.push(
    halfPlaneThroughPoint(line(1, 0, -W), new Point((W - 1) as Coordinate, 1 as Coordinate))
  )
  ...
  clipPlanes.push(
    halfPlaneThroughPoint(line(0, 1, -H), new Point(1 as Coordinate, (H - 1) as Coordinate))
  )
...
export function intersectLineAndBox(line: Line, box: Box): Segment | Point | null {
  if (line.a === 0) {
    ...
    return box.min.y <= l && l <= box.max.y
      ? lineSegment(
          new Point(box.min.x as Coordinate, l as Coordinate),
          new Point(box.max.x as Coordinate, l as Coordinate)
        )
      : null
  }
  if (line.b === 0) {
    const h = -line.c / line.a
    return box.min.x <= h && h <= box.max.x
      ? lineSegment(
          new Point(h as Coordinate, box.min.y as Coordinate),
          new Point(h as Coordinate, box.max.y as Coordinate)
        )
      : null
  }

  const points: Point[] = []
  const u = (value: number): void => {
    const i = -(line.c + line.a * value) / line.b
    if (box.min.y <= i && i <= box.max.y) {
      addPoint(points, new Point(value as Coordinate, i as Coordinate))
    }
  }
  const p = (value: number): void => {
    const s = -(line.c + line.b * value) / line.a
    if (box.min.x <= s && s <= box.max.x) {
      addPoint(points, new Point(s as Coordinate, value as Coordinate))
    }
  }
...
export function intersectLineSegments(
  ...
  const z = ((e: Point, t: Point, i: Point, s: Point): number | null => {
...
export function intersectLineSegmentAndBox(segment: Segment, box: Box): Point | Segment | null {
  ...
  return accept
    ? equalPoints(new Point(x0, y0), new Point(x1, y1))
      ? new Point(x0, y0)
      : lineSegment(new Point(x0, y0), new Point(x1, y1))
    : null
}
...
export function pointInTriangle(point: Point, end0: Point, end1: Point, end2: Point): boolean {
  ...
  return (
    intersectLineSegments(end0, end1, middle, point) === null &&
 ...
 */
export function intersectPolygonAndHalfPlane(
  points: Point[],
  halfPlane: HalfPlane
): Point[] | null {
...
function addPointToPointsSet(points: Point[], point: Point): boolean {
  ...
  if (points.length > 1 && equalPoints(points[0], point)) {
    // Check first point only if there are enough points
    return false
  }
  points.push(point)
  return true
 ...
 */
export function extendAndClipLineSegment(
  point0: Point,
  point1: Point,
  width: number,
  height: number,
  ...
  if (extendLeft) {
    ...
    return intersection === null || equalPoints(point1, intersection)
      ? null
      : lineSegment(point1, intersection)
  }
  ...
  if (extendRight) {
    ...
    return intersection === null || equalPoints(point0, intersection)
      ? null
      : lineSegment(point0, intersection)
  }
...
function sampleFromSeriesData<HorzScaleItem>(
  ...
  const logicalIndexRaw = chart
    .timeScale()
    .timeToIndex(data.time as unknown as HorzScaleItem, false)
...
export function interpolateTimeFromLogicalIndex<HorzScaleItem>(
  ...
  const leftSample = sampleAtIndex(chart, series, Math.floor(logicalIndex), MISMATCH_NEAREST_LEFT)
  const rightSample = sampleAtIndex(chart, series, Math.ceil(logicalIndex), MISMATCH_NEAREST_RIGHT)
...
export function interpolateLogicalIndexFromTime<HorzScaleItem>(
  ...
  const exactLogicalIndex = timeScale.timeToIndex(timestamp as unknown as HorzScaleItem, false)
  ...
  if (!firstSample || !lastSample) {
    console.warn(
      '[interpolateLogicalIndexFromTime] Not enough data points to reliably interpolate logical index.'
    )
    return null
  }
