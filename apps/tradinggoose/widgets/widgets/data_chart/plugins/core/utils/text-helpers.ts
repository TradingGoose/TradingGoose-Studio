...
import type { TextRendererData } from '../types'
...
export function getBoxHeight(data: TextRendererData, linesCount: number): number {
  ...
  return (
    scaledFontSize * linesCount +
    scaledPadding * (linesCount - 1) +
    2 * getScaledBackgroundInflationY(data) +
    2 * getScaledBoxPaddingY(data)
  )
}
...
export function textWrap(
  text: string,
  font: string,
  lineWrapWidth: number | string | undefined
): string[] {
  ...
  if (typeof lineWrapWidth === 'string') {
    wrapWidthNum = Number.parseInt(lineWrapWidth)
