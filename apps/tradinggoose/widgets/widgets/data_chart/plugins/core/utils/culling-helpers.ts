...
import type { ExtendOptions, LineToolCullingInfo, SinglePointOrientation } from '../types'
import { getExtendedVisiblePriceRange, interpolateTimeFromLogicalIndex } from './geometry'
...
export function getViewportBounds<HorzScaleItem>(
  ...
  const minTime = Math.ceil(minTimeNum)
...
export function getCullingStateWithExtensions(
  ...
  const t_start = extendOptions.left ? Number.NEGATIVE_INFINITY : 0
  const t_end = extendOptions.right ? Number.POSITIVE_INFINITY : 1
...
function calculateInfiniteLineClip(
  ...
  let t_enter = Number.NEGATIVE_INFINITY
  let t_exit = Number.POSITIVE_INFINITY
  ...
  if (dx === 0) {
    ...
    if (p1.timestamp < viewport.minTime || p1.timestamp > viewport.maxTime) {
      return [Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY] // RETURN MISS
    }
...
export function getToolCullingState<HorzScaleItem>(
  ...
  if (points.length >= 3 && cullingInfo?.subSegments && extendOptions) {
    ...
    for (const [indexA, indexB] of cullingInfo.subSegments) {
      ...
      if (points[indexA] && points[indexB]) {
        ...
        if (
          callTwoPointCuller(points[indexA], points[indexB], extendOptions) ===
          OffScreenState.Visible
        ) {
  ...
  if (points.length === 1) {
    ...
    if (!isHorizontalLineActive && !isVerticalLineActive) {
      ...
      if (
        p.timestamp >= viewportBounds.minTime &&
        p.timestamp <= viewportBounds.maxTime &&
        p.price >= viewportBounds.minPrice &&
        p.price <= viewportBounds.maxPrice
      ) {
        return OffScreenState.Visible
      }
    ...
    if (isVerticalLineActive) {
      ...
      const isAlignedHorizontally =
        p.timestamp >= viewportBounds.minTime && p.timestamp <= viewportBounds.maxTime
