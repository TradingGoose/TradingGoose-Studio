...
import { type Coordinate, LineStyle } from 'lightweight-charts'
import {
...
import { ensureDefined, ensureNotNull } from '../utils/helpers'
import {
  cacheCanvas,
  createCacheCanvas,
  getBoxHeight,
...
import type { AnchorPoint } from './line-anchor-renderer'
...
export class SegmentRenderer<HorzScaleItem> implements IPaneRenderer {
  ...
  public draw(target: CanvasRenderingTarget2D): void {
    ...
    target.useMediaCoordinateSpace(
      ({ context: ctx, mediaSize }: MediaCoordinatesRenderingScope) => {
...
export class PolygonRenderer<HorzScaleItem> implements IPaneRenderer {
  ...
  public hitTest(x: Coordinate, y: Coordinate): HitTestResult<LineToolHitTestData> | null {
    ...
    const { points, background, hitTestBackground, toolDefaultHoverCursor, toolDefaultDragCursor } =
      this._data // NEW: Get default cursors

    // Hit test line segments (perimeter)
    for (let i = 1; i < points.length; i++) {
      if (
        distanceToSegment(points[i - 1], points[i], point).distance <= interactionTolerance.line
      ) {
        const suggestedCursor = toolDefaultHoverCursor || PaneCursorType.Pointer
    ...
    if (
      points.length > 2 &&
      distanceToSegment(points[points.length - 1], points[0], point).distance <=
        interactionTolerance.line
    ) {
      const suggestedCursor = toolDefaultHoverCursor || PaneCursorType.Pointer
      return new HitTestResult(HitTestType.MovePoint, { pointIndex: null, suggestedCursor })
    }
...
export class RectangleRenderer<HorzScaleItem> implements IPaneRenderer {
  ...
  public hitTest(x: Coordinate, y: Coordinate): HitTestResult<LineToolHitTestData> | null {
    ...
    const { extend, points, hitTestBackground, toolDefaultHoverCursor, toolDefaultDragCursor } =
      this._data!
    const [point0, point1] = points
    ...
    if (distanceToSegment(htTopLeft, htTopRight, clickedPoint).distance <= lineTolerance) {
      return new HitTestResult(HitTestType.MovePoint, {
        pointIndex: null,
        suggestedCursor: suggestedHoverCursor,
      })
    }
    ...
    const htBottomRight = new Point(
      extend?.right ? (this._mediaSize.width as Coordinate) : maxX,
      maxY
    )
    ...
    if (hitTestBackground && pointInBox(clickedPoint, new Box(topLeft, bottomRight))) {
      ...
      return new HitTestResult(HitTestType.MovePointBackground, {
        pointIndex: null,
        suggestedCursor: suggestedDragCursor,
      })
    }
...
export class TextRenderer<HorzScaleItem> implements IPaneRenderer {
  ...
  public setData(data: TextRendererData): void {
    ...
    function checkUnchanged(
      before: TextRendererData | null,
      after: TextRendererData | null
    ): boolean {
      ...
      if (before.points !== undefined && after.points !== undefined) {
        ...
        for (let i = 0; i < before.points.length; ++i) {
          if (
            before.points[i].x !== after.points[i].x ||
            before.points[i].y !== after.points[i].y
          ) {
            return false
          }
        }
      ...
      return (
        ...
        JSON.stringify(before.text?.box?.border?.radius) ===
          JSON.stringify(after.text?.box?.border?.radius) && // For array comparison
  ...
  public isOutOfScreen(width: number, height: number): boolean {
    ...
    if (internalData.boxLeft + internalData.boxWidth < 0 || internalData.boxLeft > width) {
      const screenBox = new Box(
        new Point(0 as Coordinate, 0 as Coordinate),
        new Point(width as Coordinate, height as Coordinate)
      )
  ...
  public draw(target: CanvasRenderingTarget2D): void {
    ...
    target.useMediaCoordinateSpace(
      ({ context: ctx, mediaSize }: MediaCoordinatesRenderingScope) => {
        ...
        if (shadowApplied) {
          // Only reset if shadow was applied
        ...
        // 4. Draw border
        if (
          (textData.box?.border?.width || 0) > 0 &&
          textData.box?.border?.color &&
          !isFullyTransparent(textData.box.border.color)
        ) {
        ...
        if (alignValue === TextAlignment.Center) {
          ...
          // Catches TextAlignment.Start, TextAlignment.Left, and any ambiguous default
          canvasAlign = 'left'
        }
  ...
  private _getInternalData(): InternalData {
    ...
    const isDegenerate =
      data.points && data.points.length >= 2 && equalPoints(data.points[0], data.points[1])
    ...
    if (!data.points || data.points.length < 2 || isDegenerate) {
      ...
      const refX: number = defaultAnchor.x as number
      const refY: number = defaultAnchor.y as number
    ...
    if (null === this._linesInfo) {
      ...
      if (data.text?.box?.maxHeight !== undefined && data.text.box.maxHeight > 0) {
        ...
        if (lineHeightWithSpacing > 0) {
          ...
          maxLines = Number.POSITIVE_INFINITY // If no height/spacing per line, assume unlimited lines
        }
  ...
  private _getPolygonPoints(): Point[] {
    ...
    this._polygonPoints = [
      ...
      rotatePoint(
        new Point((boxLeft + boxWidth) as Coordinate, boxTop as Coordinate),
        pivot,
        angle
      ),
      rotatePoint(
        new Point((boxLeft + boxWidth) as Coordinate, (boxTop + boxHeight) as Coordinate),
        pivot,
        angle
      ),
      rotatePoint(
        new Point(boxLeft as Coordinate, (boxTop + boxHeight) as Coordinate),
        pivot,
        angle
      ),
    ]
  ...
  public draw(target: CanvasRenderingTarget2D): void {
    ...
    target.useMediaCoordinateSpace(
      ({ context: ctx, mediaSize }: MediaCoordinatesRenderingScope) => {
        ...
        if (radius <= 0) {
          // Don't draw if radius is zero or negative
          return
        }
