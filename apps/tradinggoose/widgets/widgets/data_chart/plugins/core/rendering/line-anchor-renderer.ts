...
import { type Coordinate, type IChartApiBase, LineStyle } from 'lightweight-charts'
import {
...
import { type DeepPartial, merge } from '../utils/helpers'
...
export class LineAnchorRenderer<HorzScaleItem> implements IPaneRenderer {
  ...
  public updateData(data: Partial<LineAnchorRendererData>): void {
    if (this._data) {
      this._data = merge(
        this._data as DeepPartial<LineAnchorRendererData>,
        data
      ) as LineAnchorRendererData
    }
  }
  ...
  private _drawPoints(
    ...
    points: AnchorPoint[],
    colors: string[],
    drawBody: DrawCallback,
    drawShadow: DrawCallback
  ): void {
