  ...
  public draw(
    ...
    target.useMediaCoordinateSpace(
      ({ context: ctx, mediaSize }: MediaCoordinatesRenderingScope) => {
        ...
        const textWidthCacheToUse =
          rendererOptions.widthCache || this._ensureFallbackTextWidthCache()
        ...
        const textWidth = Math.round(
          textWidthCacheToUse.measureText(ctx, this._data!.text, optimizationReplacementRe)
        )
        if (textWidth <= 0) {
          return
        }
