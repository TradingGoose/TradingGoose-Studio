...
import type { LineToolPriceRange } from '../model/LineToolPriceRange'
...
export class LineToolPriceRangePaneView<HorzScaleItem> extends LineToolPaneView<HorzScaleItem> {
  ...
  protected override _updateImpl(height: number, width: number): void {
    ...
    if (
      P0_cull &&
      P1_cull &&
      this._points.length >= this._tool.pointsCount &&
  ...
  private _addPriceDifferenceLabel(
    ...
    const P0_price_value = Number.parseFloat(priceFormatter.format(price0Raw.price))
    const P1_price_value = Number.parseFloat(priceFormatter.format(price1Raw.price))
    ...
    const placementVerticalAlignment = isUpward
  ...
  protected override _addAnchors(renderer: CompositeRenderer<any>): void {
    ...
    const anchorData = {
      ...
      points: [anchor0, anchor1, anchor2, anchor3, anchor4, anchor5, anchor6, anchor7],
    }
