...
export const IndicatorControl = ({
  ...
  return (
    <div
      className={cn(
        'inline-flex min-w-0 max-w-full self-start items-center h-6 gap-1 rounded-sm border border-border/40 text-center text-xs shadow-xs bg-background/40 backdrop-blur-sm hover:bg-background',
        isHidden && 'opacity-60'
      )}
      onMouseLeave={() => setIsHoveringData(false)}
    >
      <div
        className='min-w-0 inline-flex items-center gap-1 shrink'
      ...
      {!hasError && plotValues && plotValues.length > 0 && (
        <div
          className={cn(
            'pointer-events-none flex flex-nowrap min-w-0 max-w-full overflow-hidden p-0.5 items-center gap-2 text-xs text-muted-foreground shrink',
