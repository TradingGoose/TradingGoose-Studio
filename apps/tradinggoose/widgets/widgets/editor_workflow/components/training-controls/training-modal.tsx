...
export function TrainingModal({ channelId, isOpen, onClose }: TrainingModalProps) {
  ...
  const startTraining = useCopilotTrainingStore((state) => state?.startTraining ?? (() => undefined))
  const cancelTraining = useCopilotTrainingStore(
    (state) => state?.cancelTraining ?? (() => undefined)
  )
  const clearDatasets = useCopilotTrainingStore(
    (state) => state?.clearDatasets ?? (() => undefined)
  )
  const exportDatasets = useCopilotTrainingStore(
    (state) => state?.exportDatasets ?? (() => JSON.stringify({ datasets: [] }))
  )
  const markDatasetSent = useCopilotTrainingStore(
    (state) => state?.markDatasetSent ?? (() => undefined)
  )
  const stopTraining = useCopilotTrainingStore((state) => state?.stopTraining ?? (() => null))
  ...
  const handleStart = () => {
    if (localTitle.trim() && localPrompt.trim()) {
      startTraining(channelId, localTitle, localPrompt)
      setLocalTitle('')
      setLocalPrompt('')
      onClose()
    }
  ...
  const handleExportAll = () => {
    const dataStr = exportDatasets(channelId)
  ...
  const handleSendOne = (dataset: any) => {
    ...
    sendToIndexer(dataset)
      .then(() => {
        ...
        markDatasetSent(channelId, dataset.id)
  ...
  const handleSendAll = async () => {
    ...
    try {
      ...
      successfulIds.forEach((id) => markDatasetSent(channelId, id))
            ...
            <div className='mt-4 rounded-lg border bg-orange-50 p-4 dark:bg-orange-950/30'>
              ...
              <div className='flex gap-2'>
                <Button
                  variant='outline'
                  size='sm'
                  onClick={() => cancelTraining(channelId)}
                  className='flex-1'
                >
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <div className='flex gap-2'>
                    ...
                    <Button
                      variant='outline'
                      size='sm'
                      onClick={() => clearDatasets(channelId)}
                      disabled={datasets.length === 0}
                    >
