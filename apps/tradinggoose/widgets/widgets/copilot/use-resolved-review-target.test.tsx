...
const mockResolveCopilotEntityReviewTarget = vi.fn()

vi.mock('@/widgets/widgets/copilot/review-target-utils', async () => {
  const actual = await vi.importActual<
    typeof import('@/widgets/widgets/copilot/review-target-utils')
  >('@/widgets/widgets/copilot/review-target-utils')

  return {
    ...actual,
    resolveCopilotEntityReviewTarget: (...args: unknown[]) =>
      mockResolveCopilotEntityReviewTarget(...args),
  }
})
...
describe('useResolvedReviewTarget', () => {
  ...
  beforeEach(() => {
    ...
    mockResolveCopilotEntityReviewTarget.mockReset()
    mockResolveCopilotEntityReviewTarget.mockResolvedValue({
      descriptor: {
        workspaceId: 'ws-1',
        entityKind: 'skill',
        entityId: 'skill-1',
  ...
  it('does not re-resolve after unrelated state changes without a target change', async () => {
    ...
    const initialCallCount = mockResolveCopilotEntityReviewTarget.mock.calls.length
    ...
    expect(mockResolveCopilotEntityReviewTarget).toHaveBeenCalledTimes(initialCallCount)
  ...
  it('resolves the requested entity id with read access', async () => {
    mockResolveCopilotEntityReviewTarget.mockResolvedValueOnce({
      descriptor: {
        workspaceId: 'ws-1',
        entityKind: 'skill',
        entityId: 'skill-2',
    ...
    expect(mockResolveCopilotEntityReviewTarget).toHaveBeenCalledWith({
      workspaceId: 'ws-1',
      entityKind: 'skill',
      entityId: 'skill-2',
      accessMode: 'read',
