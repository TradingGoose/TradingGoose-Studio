 ...
 * and total_thought_tokens (for thinking models).
 *
      ...
      try {
        for await (const event of stream) {
          if (event.event_type === 'content.delta') {
            ...
            const interaction = (event as Interactions.InteractionCompleteEvent).interaction
...
export async function executeGeminiRequest(
  ...
  try {
    ...
    if (functionCalls?.length) {
      const functionNames = functionCalls.map((fc: { name?: string }) => fc.name ?? '').join(', ')
