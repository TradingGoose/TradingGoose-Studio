...
Una vez procesados los documentos, añade un bloque Knowledge, selecciona una base de conocimiento y configura una consulta. Para usar los resultados en generación aumentada por recuperación (RAG), referencia explícitamente los resultados del bloque en el prompt del Agent posterior, por ejemplo `<knowledge.results>` si el bloque se llama `knowledge`. Conectar los bloques no inyecta el contenido recuperado. También puedes configurar Knowledge como herramienta que el Agent puede invocar.

**Limitación actual de los filtros de etiquetas:** El bloque Knowledge no envía los filtros en el formato que acepta la API de búsqueda. Las solicitudes solo con etiquetas fallan, y las solicitudes con consulta y etiquetas se ejecutan sin esos filtros. No dependas de los filtros del bloque para restringir el contenido recuperado. Consulta [Etiquetas y filtrado](/knowledgebase/tags) para conocer los modos de búsqueda compatibles con la API.
