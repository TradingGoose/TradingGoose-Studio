...
**Limitación actual del bloque:** El bloque Knowledge envía `tagFilters` en lugar del campo `filters` de la API. Una solicitud del bloque solo con etiquetas falla la validación; una solicitud con consulta y etiquetas se ejecuta sin los filtros de etiquetas. Hasta que se corrija este contrato, usa el bloque para búsquedas solo con consulta y no dependas de sus campos de etiquetas para restringir contenido. Los modos siguientes describen el comportamiento de la API cuando los filtros se envían correctamente.
...
2. **Ingresar consulta**: Proporciona una consulta mientras exista la limitación de filtros del bloque
...
- **Etiquetas variables**: Un cliente de la API puede enviar valores resueltos dinámicamente mediante `filters`; sigue aplicándose la limitación del bloque Knowledge descrita arriba
...
4. **Integra en tus flujos de trabajo**: Verifica que el cliente envíe el campo `filters` de la API; no dependas de los campos de etiquetas del bloque, que actualmente no funcionan
