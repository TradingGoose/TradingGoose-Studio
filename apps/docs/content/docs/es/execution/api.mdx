...
Puedes generar claves de API desde la configuración de usuario en el panel de control de TradingGoose.
...
Recibe notificaciones en tiempo real cuando se completan las ejecuciones de flujos de trabajo. Los webhooks se configuran a través de la interfaz de usuario de TradingGoose en el editor de flujos de trabajo.

### Configuración

Los webhooks pueden configurarse para cada flujo de trabajo desde la interfaz del editor. Haz clic en el icono de webhook de la barra de control para abrir el panel de configuración, donde puedes añadir, editar y gestionar tus suscripciones.
...
Cuando se completa la ejecución de un flujo de trabajo, TradingGoose envía una solicitud POST a tu URL de webhook:
...
- `tradinggoose-delivery-id`: ID único de entrega para idempotencia
...
Si configuras un secreto de webhook, verifica la firma para asegurar que el webhook proviene de TradingGoose:
...
2. **Seguridad de webhook**: Siempre configura un secreto de webhook y verifica las firmas para asegurar que las solicitudes provienen de TradingGoose.
