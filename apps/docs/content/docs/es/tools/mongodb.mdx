...
Ejecutar operación de búsqueda en una colección de MongoDB
...
| `sort` | string | No | Criterios de ordenamiento como cadena JSON |
...
Insertar documentos en una colección de MongoDB
...
| `insertedId` | string | ID del documento insertado \(inserción individual\) |
...
| --------- | ---- | ----------- |
| `message` | string | Mensaje de estado de la operación |
| `matchedCount` | number | Número de documentos coincidentes con el filtro |
...
| `authSource` | string | No | Base de datos de autenticación |
| `ssl` | string | No | Modo de conexión SSL \(disabled, required, preferred\) |
| `collection` | string | Sí | Nombre de la colección sobre la que ejecutar el pipeline |
...
Integra MongoDB en el flujo de trabajo. Permite buscar, insertar, actualizar, eliminar y agregar datos.
