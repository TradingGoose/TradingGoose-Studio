...
- `path.to.data` es la ruta al campo de datos específico, separada por puntos
...
Además de las referencias a bloques, el resolutor admite varios prefijos especiales:

- **Alias de triggers**: `<start.input>`, `<api.fieldName>`, `<chat.input>`, `<manual.input>` - referencias abreviadas a las salidas del bloque trigger
- **Variables**: `<variable.myVar>` - hace referencia a variables del workflow por nombre
- **Bucles**: `<loop.item>`, `<loop.index>` - hace referencia al elemento o índice actual dentro de un bucle
- **Ejecuciones paralelas**: `<parallel.item>`, `<parallel.index>` - hace referencia al elemento o índice actual dentro de una ejecución paralela
...
4. Admiten índices de arrays con notación de corchetes (por ejemplo, `items[0]`)
