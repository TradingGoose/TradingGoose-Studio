  ...
  <BlockConfigPreview
    ...
    subBlocks={[
        ...
        },
        {
            ...
            "title": "API 密钥",
            "layout": "full",
            "placeholder": "请输入您的 Google API 密钥",
            "password": true,
            "required": true
        },
        {
...
| `searchEngineId` | string | 是 | 自定义搜索引擎 ID |
| `num` | string | 否 | 返回结果数量 \(默认值：10，最大值：10\) |
...
| `title` | string | 搜索结果标题 |
| `link` | string | 搜索结果URL |
| `snippet` | string | 搜索结果摘要或描述 |
| `displayLink` | string | 显示URL |

## 使用说明

将Google Search集成到工作流中，可搜索互联网内容。
