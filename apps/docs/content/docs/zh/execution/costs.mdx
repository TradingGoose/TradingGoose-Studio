...
TradingGoose 会自动计算所有工作流执行的成本，提供基于 AI 模型使用和执行费用的透明定价。了解这些成本有助于您优化工作流并有效管理预算。
...
<Tabs items={['托管模型', '自带 API 密钥']}>
  <Tab>
    **托管模型** - TradingGoose 提供 API 密钥，价格有 2.5 倍的加成：
...
- 在 [设置 → 订阅](https://tradinggoose.ai/settings/subscription) 中查看您当前的使用情况
...
当未结算的超额费用达到 $50 时，TradingGoose 会自动结算全部未结算金额。
