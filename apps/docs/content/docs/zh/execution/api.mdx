...
TradingGoose 提供了一个全面的外部 API，用于查询工作流执行日志，并在工作流完成时设置实时通知的 webhook。
...
您可以在 TradingGoose 仪表板的用户设置中生成 API 密钥。
...
在工作流执行完成时获取实时通知。Webhook 可通过 TradingGoose UI 的工作流编辑器进行配置。

### 配置

可以在工作流编辑器界面中为每个工作流配置 Webhook。点击控制栏中的 Webhook 图标，打开配置面板并添加、编辑和管理 Webhook 订阅。
...
当工作流执行完成时，TradingGoose 会向您的 Webhook URL 发送一个 POST 请求：
...
- `tradinggoose-timestamp`：以毫秒为单位的 Unix 时间戳
- `tradinggoose-delivery-id`：用于幂等性的唯一交付 ID
- `tradinggoose-signature`：用于验证的 HMAC-SHA256 签名（如果配置了密钥）
- `Idempotency-Key`：与交付 ID 相同，用于检测重复

### 签名验证

如果您配置了 webhook 密钥，请验证签名以确保 webhook 来自 TradingGoose：
...
2. **Webhook 安全性**：始终配置 webhook 密钥并验证签名以确保请求来自 TradingGoose。
