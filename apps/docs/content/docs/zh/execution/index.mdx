...
TradingGoose 的执行引擎通过按正确的顺序处理模块、管理数据流并优雅地处理错误，使您的工作流栩栩如生，从而让您能够准确了解工作流在 TradingGoose 中的执行方式。
...
当您将工作流部署为 API 时，TradingGoose 会：
- 创建一个唯一的 HTTP 端点：`https://tradinggoose.ai/api/workflows/{workflowId}/execute`
- 生成一个用于身份验证的 API 密钥
...
聊天部署为您的工作流创建一个对话界面：
- 托管在自定义子域上：`https://your-name.tradinggoose.ai`
- 可选身份验证（公开、密码或基于电子邮件）
- 可使用您的品牌自定义界面
- 流式响应实现实时交互
...
Deploy 模态框通过部署快照表保留完整的版本历史记录。你可以检查任意快照、将其与草稿比较，并在需要恢复旧版本时一键提升或回滚。
