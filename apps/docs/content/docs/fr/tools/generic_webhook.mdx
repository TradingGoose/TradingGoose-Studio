...
Le bloc Webhook générique vous permet de recevoir des webhooks depuis n'importe quel service externe. C'est un déclencheur flexible qui peut traiter n'importe quelle charge utile JSON, ce qui le rend idéal pour l'intégration avec des services qui n'ont pas de bloc TradingGoose dédié.
...
```bash
curl -X POST https://tradinggoose.ai/api/webhooks/trigger/{webhook-path} \
  -H "Content-Type: application/json" \
  -H "X-TradingGoose-Secret: your-secret" \
  -d '{
...
3. Choisissez le type d'en-tête :
   - **En-tête personnalisé** : `X-TradingGoose-Secret: your-token`
...
curl -X POST https://tradinggoose.ai/api/webhooks/trigger/{webhook-path} \
  -H "Content-Type: application/json" \
  -H "X-TradingGoose-Secret: your-secret-token" \
