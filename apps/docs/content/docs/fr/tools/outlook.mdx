...
Dans TradingGoose, l'intégration de Microsoft Outlook permet à vos agents d'interagir directement avec les données d'e-mail et de calendrier de manière programmatique. Cela permet des scénarios d'automatisation puissants tels que l'envoi de mises à jour personnalisées par e-mail, l'analyse des messages entrants pour déclencher des flux de travail, la création d'événements de calendrier et la gestion des rappels de tâches. En connectant TradingGoose à Microsoft Outlook, vous permettez aux agents intelligents d'automatiser les communications, de rationaliser la planification et de maintenir la visibilité sur la correspondance organisationnelle — le tout au sein de votre écosystème de flux de travail.
{/* MANUAL-CONTENT-END */}

## Instructions d'utilisation

Intégrez Outlook dans le flux de travail. Peut lire, rédiger et envoyer des messages électroniques. Nécessite OAuth. Peut être utilisé en mode déclencheur pour lancer un flux de travail lorsqu'un nouvel e-mail est reçu.

## Outils

### `outlook_send`

Envoyer des e-mails avec Outlook
...
Rédiger des e-mails avec Outlook
...
| `messageId` | string | Identifiant unique pour l'e-mail en brouillon |
| `status` | string | Statut du brouillon de l'e-mail |
| `subject` | string | Objet de l'e-mail en brouillon |
| `timestamp` | string | Horodatage de création du brouillon |
| `message` | string | Message de réussite ou d'erreur |

### `outlook_read`

Lire des e-mails depuis Outlook
...
Transférer un message Outlook existant aux destinataires spécifiés
