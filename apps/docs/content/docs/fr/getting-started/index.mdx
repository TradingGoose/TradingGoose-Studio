...
Ce tutoriel vous guidera dans la création de votre premier flux de travail IA dans TradingGoose. Nous allons créer un agent de recherche de personnes capable de trouver des informations sur des individus en utilisant des outils de recherche LLM à la pointe de la technologie.

<Callout type="info">
  Ce tutoriel prend environ 10 minutes et couvre les concepts essentiels de la création de flux de travail dans TradingGoose.
</Callout>
...
<Steps>
  <Step title="Créer un flux de travail et ajouter un agent IA">
    Ouvrez TradingGoose et cliquez sur « Nouveau flux de travail » dans le tableau de bord. Nommez-le « Premiers pas ».
