...
TradingGoose calcule automatiquement les coûts pour toutes les exécutions de flux de travail, offrant une tarification transparente basée sur l'utilisation des modèles d'IA et les frais d'exécution. Comprendre ces coûts vous aide à optimiser les flux de travail et à gérer efficacement votre budget.
...
<Tabs items={['Modèles hébergés', 'Apportez votre propre clé API']}>
  <Tab>
    **Modèles hébergés** - TradingGoose fournit des clés API avec un multiplicateur de prix de 2,5x :
...
- Examinez votre utilisation actuelle dans [Paramètres → Abonnement](https://tradinggoose.ai/settings/subscription)
...
Lorsque le dépassement non facturé atteint 50 $, TradingGoose facture automatiquement le montant total non facturé.
...
- Consultez votre utilisation actuelle dans [Paramètres → Abonnement](https://tradinggoose.ai/settings/subscription)
