...
Le moteur d'exécution de TradingGoose donne vie à vos flux de travail en traitant les blocs dans le bon ordre, en gérant le flux de données et en traitant les erreurs avec élégance, afin que vous puissiez comprendre exactement comment les flux de travail sont exécutés dans TradingGoose.
...
Lorsque vous déployez un workflow en tant qu'API, TradingGoose :
- Crée un point de terminaison HTTP unique : `https://tradinggoose.ai/api/workflows/{workflowId}/execute`
...
- Hébergée sur un sous-domaine personnalisé : `https://your-name.tradinggoose.ai`
