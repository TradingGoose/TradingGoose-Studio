...
TradingGoose fournit une API externe complète pour interroger les journaux d'exécution des workflows et configurer des webhooks pour des notifications en temps réel lorsque les workflows sont terminés.
...
Vous pouvez générer des clés API depuis vos paramètres utilisateur dans le tableau de bord TradingGoose.
...
Recevez des notifications en temps réel lorsque les exécutions de workflow sont terminées. Les webhooks sont configurés via l'interface utilisateur TradingGoose dans l'éditeur de workflow.
...
Lorsqu'une exécution de workflow est terminée, TradingGoose envoie une requête POST à votre URL webhook :
...
- `tradinggoose-event` : Type d'événement (toujours `workflow.execution.completed`)
- `tradinggoose-timestamp` : Horodatage Unix en millisecondes
- `tradinggoose-delivery-id` : ID de livraison unique pour l'idempotence
- `tradinggoose-signature` : Signature HMAC-SHA256 pour vérification (si un secret est configuré)
- `Idempotency-Key` : Identique à l'ID de livraison pour la détection des doublons

### Vérification de signature

Si vous configurez un secret webhook, vérifiez la signature pour vous assurer que le webhook provient de TradingGoose :
...
2. **Sécurité des webhooks** : configurez toujours un secret webhook et vérifiez les signatures pour vous assurer que les requêtes proviennent de TradingGoose.
