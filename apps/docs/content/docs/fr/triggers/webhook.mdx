...
Les webhooks permettent aux services externes de déclencher l'exécution de flux de travail à partir de webhooks externes en envoyant des requêtes HTTP à votre flux de travail. TradingGoose prend en charge deux approches pour les déclencheurs basés sur les webhooks.
