---
title: Jira
description: Interact with Jira
---

import { BlockInfoCard } from "@/components/ui/block-info-card"
import { BlockConfigPreview } from "@/components/ui/block-config-preview"
import { ShowcaseCard } from "@/components/ui/showcase-card"


<BlockInfoCard
  type="jira"
  color="#2684FF"
/>


## Configuration

### Read Issue

<ShowcaseCard>
  <BlockConfigPreview
    name="Jira"
    type="jira"
    color="#2684FF"
    hideHeader
    subBlocks={[
        {
            "id": "operation",
            "type": "dropdown",
            "title": "Operation",
            "layout": "full",
            "defaultValue": "read",
            "options": [
                {
                    "id": "read",
                    "label": "Read Issue"
                },
                {
                    "id": "update",
                    "label": "Update Issue"
                },
                {
                    "id": "write",
                    "label": "Write Issue"
                }
            ]
        },
        {
            "id": "domain",
            "type": "short-input",
            "title": "Domain",
            "layout": "full",
            "placeholder": "Enter Jira domain (e.g., tradinggoose.atlassian.net)",
            "required": true
        },
        {
            "id": "credential",
            "type": "oauth-input",
            "title": "Jira Account",
            "layout": "full",
            "placeholder": "Select Jira account",
            "provider": "jira",
            "required": true
        },
        {
            "id": "projectId",
            "type": "project-selector",
            "title": "Select Project",
            "layout": "full",
            "placeholder": "Select Jira project",
            "provider": "jira"
        },
        {
            "id": "manualProjectId",
            "type": "short-input",
            "title": "Project ID",
            "layout": "full",
            "placeholder": "Enter Jira project ID"
        },
        {
            "id": "issueKey",
            "type": "file-selector",
            "title": "Select Issue",
            "layout": "full",
            "placeholder": "Select Jira issue",
            "provider": "jira"
        },
        {
            "id": "manualIssueKey",
            "type": "short-input",
            "title": "Issue Key",
            "layout": "full",
            "placeholder": "Enter Jira issue key"
        }
    ]}
  />
</ShowcaseCard>

<div className="mt-6 border-t border-fd-border pt-4">
<div className="text-base font-semibold mb-2">Read Issue (<code>jira_retrieve</code>)</div>

Retrieve detailed information about a specific Jira issue

<div className="text-sm font-medium text-fd-muted-foreground mt-4 mb-2">Input</div>

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `domain` | string | Yes | Your Jira domain \(e.g., yourcompany.atlassian.net\) |
| `projectId` | string | No | Jira project ID \(optional; not required to retrieve a single issue\). |
| `issueKey` | string | Yes | Jira issue key to retrieve \(e.g., PROJ-123\) |
| `cloudId` | string | No | Jira Cloud ID for the instance. If not provided, it will be fetched using the domain. |

<div className="text-sm font-medium text-fd-muted-foreground mt-4 mb-2">Output</div>

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `success` | boolean | Operation success status |
| `output` | object | Jira issue details with issue key, summary, description, created and updated timestamps |

</div>



---

### Update Issue

<ShowcaseCard>
  <BlockConfigPreview
    name="Jira"
    type="jira"
    color="#2684FF"
    hideHeader
    subBlocks={[
        {
            "id": "operation",
            "type": "dropdown",
            "title": "Operation",
            "layout": "full",
            "defaultValue": "update",
            "options": [
                {
                    "id": "read",
                    "label": "Read Issue"
                },
                {
                    "id": "update",
                    "label": "Update Issue"
                },
                {
                    "id": "write",
                    "label": "Write Issue"
                }
            ]
        },
        {
            "id": "domain",
            "type": "short-input",
            "title": "Domain",
            "layout": "full",
            "placeholder": "Enter Jira domain (e.g., tradinggoose.atlassian.net)",
            "required": true
        },
        {
            "id": "credential",
            "type": "oauth-input",
            "title": "Jira Account",
            "layout": "full",
            "placeholder": "Select Jira account",
            "provider": "jira",
            "required": true
        },
        {
            "id": "projectId",
            "type": "project-selector",
            "title": "Select Project",
            "layout": "full",
            "placeholder": "Select Jira project",
            "provider": "jira"
        },
        {
            "id": "manualProjectId",
            "type": "short-input",
            "title": "Project ID",
            "layout": "full",
            "placeholder": "Enter Jira project ID"
        },
        {
            "id": "issueKey",
            "type": "file-selector",
            "title": "Select Issue",
            "layout": "full",
            "placeholder": "Select Jira issue",
            "provider": "jira"
        },
        {
            "id": "manualIssueKey",
            "type": "short-input",
            "title": "Issue Key",
            "layout": "full",
            "placeholder": "Enter Jira issue key"
        },
        {
            "id": "summary",
            "type": "short-input",
            "title": "New Summary",
            "layout": "full",
            "placeholder": "Enter new summary for the issue",
            "required": true
        },
        {
            "id": "description",
            "type": "long-input",
            "title": "New Description",
            "layout": "full",
            "placeholder": "Enter new description for the issue"
        }
    ]}
  />
</ShowcaseCard>

<div className="mt-6 border-t border-fd-border pt-4">
<div className="text-base font-semibold mb-2">Update Issue (<code>jira_update</code>)</div>

Update a Jira issue

<div className="text-sm font-medium text-fd-muted-foreground mt-4 mb-2">Input</div>

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `domain` | string | Yes | Your Jira domain \(e.g., yourcompany.atlassian.net\) |
| `projectId` | string | No | Jira project ID to update issues in. If not provided, all issues will be retrieved. |
| `issueKey` | string | Yes | Jira issue key to update |
| `summary` | string | No | New summary for the issue |
| `description` | string | No | New description for the issue |
| `status` | string | No | New status for the issue |
| `priority` | string | No | New priority for the issue |
| `assignee` | string | No | New assignee for the issue |
| `cloudId` | string | No | Jira Cloud ID for the instance. If not provided, it will be fetched using the domain. |

<div className="text-sm font-medium text-fd-muted-foreground mt-4 mb-2">Output</div>

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `ts` | string | Timestamp of the operation |
| `issueKey` | string | Updated issue key \(e.g., PROJ-123\) |
| `summary` | string | Issue summary after update |

</div>



---

### Write Issue

<ShowcaseCard>
  <BlockConfigPreview
    name="Jira"
    type="jira"
    color="#2684FF"
    hideHeader
    subBlocks={[
        {
            "id": "operation",
            "type": "dropdown",
            "title": "Operation",
            "layout": "full",
            "defaultValue": "write",
            "options": [
                {
                    "id": "read",
                    "label": "Read Issue"
                },
                {
                    "id": "update",
                    "label": "Update Issue"
                },
                {
                    "id": "write",
                    "label": "Write Issue"
                }
            ]
        },
        {
            "id": "domain",
            "type": "short-input",
            "title": "Domain",
            "layout": "full",
            "placeholder": "Enter Jira domain (e.g., tradinggoose.atlassian.net)",
            "required": true
        },
        {
            "id": "credential",
            "type": "oauth-input",
            "title": "Jira Account",
            "layout": "full",
            "placeholder": "Select Jira account",
            "provider": "jira",
            "required": true
        },
        {
            "id": "projectId",
            "type": "project-selector",
            "title": "Select Project",
            "layout": "full",
            "placeholder": "Select Jira project",
            "provider": "jira"
        },
        {
            "id": "manualProjectId",
            "type": "short-input",
            "title": "Project ID",
            "layout": "full",
            "placeholder": "Enter Jira project ID"
        },
        {
            "id": "summary",
            "type": "short-input",
            "title": "New Summary",
            "layout": "full",
            "placeholder": "Enter new summary for the issue",
            "required": true
        },
        {
            "id": "description",
            "type": "long-input",
            "title": "New Description",
            "layout": "full",
            "placeholder": "Enter new description for the issue"
        }
    ]}
  />
</ShowcaseCard>

<div className="mt-6 border-t border-fd-border pt-4">
<div className="text-base font-semibold mb-2">Write Issue (<code>jira_write</code>)</div>

Write a Jira issue

<div className="text-sm font-medium text-fd-muted-foreground mt-4 mb-2">Input</div>

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `domain` | string | Yes | Your Jira domain \(e.g., yourcompany.atlassian.net\) |
| `projectId` | string | Yes | Project ID for the issue |
| `summary` | string | Yes | Summary for the issue |
| `description` | string | No | Description for the issue |
| `priority` | string | No | Priority ID or name for the issue \(e.g., "10000" or "High"\) |
| `assignee` | string | No | Assignee account ID for the issue |
| `labels` | array | No | Labels for the issue \(array of label names\) |
| `duedate` | string | No | Due date for the issue \(format: YYYY-MM-DD\) |
| `reporter` | string | No | Reporter account ID for the issue |
| `environment` | string | No | Environment information for the issue |
| `customFieldId` | string | No | Custom field ID \(e.g., customfield_10001\) |
| `customFieldValue` | string | No | Value for the custom field |

<div className="text-sm font-medium text-fd-muted-foreground mt-4 mb-2">Output</div>

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `ts` | string | Timestamp of the operation |
| `issueKey` | string | Created issue key \(e.g., PROJ-123\) |
| `summary` | string | Issue summary |
| `url` | string | URL to the created issue |
| `assigneeId` | string | Account ID of the assigned user \(if assigned\) |

</div>



---

## Additional Tools

Additional tools declared by this block, including input-dependent variants of the operations above.

### `jira_bulk_read`

Retrieve multiple Jira issues in bulk

#### Input

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `domain` | string | Yes | Your Jira domain \(e.g., yourcompany.atlassian.net\) |
| `projectId` | string | Yes | Jira project ID |
| `cloudId` | string | No | Jira cloud ID |

#### Output

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `issues` | array | Array of Jira issues with summary, description, created and updated timestamps |


## Usage Instructions

Integrate Jira into the workflow. Can read, write, and update issues.



## Notes

- Category: `tools`
- Type: `jira`
