---
title: Trading Order History
description: Retrieve recorded trading order submissions for a date range.
---

import { BlockInfoCard } from "@/components/ui/block-info-card"
import { BlockConfigPreview } from "@/components/ui/block-config-preview"
import { ShowcaseCard } from "@/components/ui/showcase-card"


<BlockInfoCard
  type="trading_order_history"
  color="#0f766e"
/>


## Configuration

<ShowcaseCard>
  <BlockConfigPreview
    name="Trading Order History"
    type="trading_order_history"
    color="#0f766e"
    hideHeader
    subBlocks={[
        {
            "id": "startDate",
            "type": "datetime-input",
            "title": "Start Date",
            "layout": "full",
            "placeholder": "YYYY-MM-DDTHH:mm:ssZ",
            "required": true
        },
        {
            "id": "endDate",
            "type": "datetime-input",
            "title": "End Date",
            "layout": "full",
            "placeholder": "YYYY-MM-DDTHH:mm:ssZ",
            "required": true
        },
        {
            "id": "workflowId",
            "type": "short-input",
            "title": "Workflow ID",
            "layout": "full",
            "placeholder": "Defaults to current workflow"
        }
    ]}
  />
</ShowcaseCard>

## Tools

### `trading_order_history`

Retrieve workspace order submissions within a datetime range.

#### Input

| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `startDate` | string | Yes | Start datetime \(ISO 8601\). |
| `endDate` | string | Yes | End datetime \(ISO 8601\). |

#### Output

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `id` | string | Order history record ID |
| `provider` | string | Trading provider |
| `recordedAt` | string | Recorded timestamp |
| `logId` | string | Linked workflow log ID |


## Usage Instructions

Fetches order submission history recorded by the Trading Action tool for the workspace and date range.



## Notes

- Category: `tools`
- Type: `trading_order_history`
