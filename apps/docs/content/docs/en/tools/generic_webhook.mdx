...
The Generic Webhook block allows you to receive webhooks from any external service. This is a flexible trigger that can handle any JSON payload, making it ideal for integrating with services that don't have a dedicated TradingGoose block.
...
3. Choose header type:
   - **Custom Header**: `X-TradingGoose-Secret: your-token`
