...
- **Live responses** -- workflow responses stream into the conversation in real time
...
- **Workflow selector** -- a dropdown in the header center lets you switch between workflows in the workspace; when the widget is paired (non-gray pair color), choosing a workflow also updates the linked widgets
- **Explicit workflow selection** -- select a workflow in this widget or a linked widget; the selection persists across sessions, with no automatic fallback to the first workflow

## Usage

### Adding the widget

Open the widget picker and select **Workflow Chat**. The widget requires an active workspace; if none is selected it displays a prompt to choose one. Once a workspace is loaded, select a workflow from the header or a linked widget. Until then, the chat shows a prompt to select a workflow.
