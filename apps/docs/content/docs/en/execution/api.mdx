...
TradingGoose provides a comprehensive external API for querying workflow execution logs and setting up webhooks for real-time notifications when workflows complete.
...
Get real-time notifications when workflow executions complete. Webhooks are configured through the TradingGoose UI in the workflow editor.

### Configuration

Webhooks can be configured for each workflow through the workflow editor UI. Click the webhook icon in the control bar to open the webhook configuration panel where you can add, edit, and manage your webhook subscriptions.
