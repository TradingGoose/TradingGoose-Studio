...
TradingGoose provides comprehensive logging for all workflow executions, giving you complete visibility into how your workflows run, what data flows through them, and where issues might occur.

## Logging System

TradingGoose offers two complementary logging interfaces to match different workflows and use cases:

### Real-Time Console

During manual or workspace-chat execution, logs appear in a [Workflow Console](/widgets/workflow-console) widget configured for the same workflow. You can place and resize that widget anywhere in a dashboard layout.
...
- Log retention based on the effective tier of the workspace's configured user or organization billing owner

## Log Details Sidebar

Clicking on any log entry opens a detailed sidebar view showing the execution timeline, block-level details, and cost summary.
...
<Tabs items={['Output', 'Input']}>
  ...
  <Tab>
    **Input Tab** displays what was passed to the block:
    ...
    - Credential-shaped values are redacted in the frozen workflow snapshot; other logged tool arguments and outputs should still be treated as potentially sensitive
  </Tab>
</Tabs>
...
For any logged execution, click "View Snapshot" to see the exact workflow state at execution time. The snapshot provides:
...
<Callout type="info">
  If a stored workflow snapshot is unavailable, the snapshot view shows a "Logged State Not Found" message.
</Callout>

## Log Retention

Log retention is configured by the effective tier of the workspace's billing owner. A finite `logRetentionDays` value determines when old logs are removed; a tier with no retention limit keeps logs indefinitely. Check the current Subscription settings rather than relying on fixed plan names or durations.
