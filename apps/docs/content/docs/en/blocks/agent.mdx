---
title: Agent
description: Build an agent using AI models with tool integration
---
...
The Agent block supports multiple LLM providers through a unified inference interface. Its model selector is grouped by provider and reflects the deployment's current built-in catalog together with models discovered from configured OpenRouter and Ollama connections. Use the selector as the source of truth instead of relying on a fixed model list.
...
When this field is shown, enter an API key for the selected provider. Hosted models on the managed deployment and local Ollama models do not show this field.
...
2. Select a built-in integration, connected MCP tool, or custom tool
...
- **Force**: Provider is instructed to use this tool in the response
- **None**: Tool is excluded from the provider request

### Response Format

The Response Format parameter requests structured output using a JSON Schema:
...
When the model returns valid JSON, the schema fields are exposed directly at the Agent block's output root. If the response is malformed, the block falls back to its standard output and includes `_responseFormatWarning`.

### Accessing Results

Without a response format, an Agent exposes these standard outputs:

- **`<agent.content>`**: The agent's response text
- **`<agent.model>`**: The model used for the response
- **`<agent.tokens>`**: Provider-reported token usage statistics
- **`<agent.toolCalls>`**: Details of any tools the agent used during execution
- **`<agent.toolResults>`**: Results returned by those tools
- **`<agent.cost>`**: Estimated cost of the API call (if available)

With a valid response format, reference each schema field directly, such as `<agent.sentiment>`. Token, tool, timing, and cost metadata remain available alongside those fields.

## Advanced Features

### Memory + Agent: Conversation History

An Agent can use an existing message array from `<memory.memories>` in its Memories field. However, the Memory block's Add operation currently fails, so the usual save-user-message → Agent → save-reply flow is not operational. See the [Memory limitation](/blocks/memory) before designing cross-run conversation persistence.
...
<Tabs items={['Configuration', 'Variables', 'Results']}>
  ...
  <Tab>
    <ul className="list-disc space-y-2 pl-6">
      ...
      <li>
        <strong>agent.toolCalls</strong>: Tool execution details and count
      </li>
      <li>
        <strong>agent.toolResults</strong>: Results returned by invoked tools
      </li>
    </ul>
  </Tab>
  <Tab>
...
<div className="mb-4 rounded-md border p-4">
  ...
  <ol className="list-decimal pl-5 text-sm">
    <li>User submits a support ticket via the API trigger</li>
    ...
    <li>Use persistent storage for future conversation history; Memory Add is currently unavailable</li>
  </ol>
</div>
