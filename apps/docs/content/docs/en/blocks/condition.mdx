---
title: Condition
description: Branch workflow paths based on boolean expressions
---
...
- **`<condition.conditionResult>`**: Whether a connected route was selected, including Else; not the truth value of the first expression.
...
Invalid expressions and unsafe property access can fail the block. Validate optional properties explicitly, for example `((<api.data>)?.tags ?? []).includes('urgent')` when the API data is an object. Condition blocks do not support the normal block error-path recovery. If validation needs recoverable exceptions, perform it in an upstream Function block with an error path.
...
<Tabs items={['Configuration', 'Variables', 'Results']}>
  ...
  <Tab>
    <ul className="list-disc space-y-2 pl-6">
      <li>
        <strong>condition.conditionResult</strong>: True when a connected route is selected, including Else
      </li>
      <li>
        <strong>condition.selectedOption</strong>: ID of the matched condition
      </li>
      <li>
        <strong>condition.selectedPath</strong>: Details of chosen routing destination
      </li>
    </ul>
  </Tab>
  <Tab>
...
<div className="mb-4 rounded-md border p-4">
  ...
  <ol className="list-decimal pl-5 text-sm">
    ...
    <li>Condition checks if `<api.data.priority>` equals 'high'</li>
...
<div className="mb-4 rounded-md border p-4">
  ...
  <ol className="list-decimal pl-5 text-sm">
    ...
    <li>Condition checks if `<function.result.account_type>` === 'enterprise'</li>
