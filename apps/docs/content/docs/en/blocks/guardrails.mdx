---
title: Guardrails
description: Validate content with safety constraints
---
...
<Steps>
  ...
  <Step>
    <strong>Detect PII</strong>: Identify and optionally mask personally identifiable information across the supported entity types
  </Step>
</Steps>
...
Detects personally identifiable information using Microsoft Presidio. Supports the selectable entity types across multiple countries and languages.
...
- **Mode**: 
  - **Block**: Detect PII and set `passed` to `false` when found (default). Connect a Condition block to enforce the decision.
  - **Mask**: Replace detected PII with masked values
- **Language**: Detection language (default: English)

**Output:**
- `passed`: in Block mode, `false` when selected PII is found; in Mask mode, `true` after masking, even when entities are detected
