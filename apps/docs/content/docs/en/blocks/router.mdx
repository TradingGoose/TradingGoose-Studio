---
title: Router
description: Intelligently route requests using AI
---
...
The Router selects from its connected destination blocks. Give those blocks clear names and explain the routing criteria in the Router prompt. To remove a destination from consideration, remove its connection; there is no separate per-target exclusion setting.

### Model Selection

Choose from the models available in the current model selector and configure that provider's credentials. Availability depends on your instance configuration.
...
- **`<router.prompt>`**: The original routing prompt
...
<Tabs items={['Configuration', 'Variables']}>
  ...
  <Tab>
    <ul className="list-disc space-y-2 pl-6">
      <li>
        <strong>router.prompt</strong>: Original routing prompt
      </li>
      <li>
        <strong>router.selectedPath</strong>: Details of chosen destination
      </li>
      <li>
