...
Block outputs and the current item are tracked per instance, but workflow variables are shared across instances. Concurrent writes to the same variable can race; return per-item outputs and aggregate them after the Parallel block.

Uncaught block failures can fail execution; parallelism is not an error-isolation guarantee. Connect error paths inside the container when recovery is required.
...
<Callout type="info">
  ...
  - Count-based execution is capped at 20 instances; collection-based execution creates an instance for every item and does not use this cap. Batch large collections explicitly.
</Callout>
...
| Order | Completion order varies; results follow instance index | Maintains order |
