...
<Tabs items={['Search', 'Upload Chunk', 'Create Document']}>
  <Tab>
    ...
    | **Search Query** | Yes for working searches | Natural-language query; see the tag-filter limitation below. |
    ...
    Tag filtering is an intended capability, but currently tag-only searches fail and query-plus-tag searches do not apply the tags. Do not rely on tags to restrict results until this is fixed.
  </Tab>
  <Tab>
...
| Operation | Output |
|---|---|
| Search | `results` (matching chunks), `query`, `totalResults` |
| Upload Chunk | `message`, `data.chunkId`, `data.content`, `data.chunkIndex`, plus `documentId`, `documentName`, and `cost` |
| Create Document | `message` and `data` containing `documentId`, `documentName`, `type`, `enabled`, `createdAt`, and `updatedAt` |

Use `<knowledge.data.documentId>` after Create Document. Write confirmations are not returned in `results`.
...
<Callout type="warning">
  Tag filtering is currently unreliable: tag-only searches fail, and query-plus-tag searches are not tag-filtered. See [Knowledgebase](/knowledgebase). Use a non-empty query and inspect returned documents; tags are not an access-control boundary.
</Callout>
