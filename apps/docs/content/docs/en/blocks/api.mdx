---
title: API
description: Connect to external services via HTTP requests
---
...
4. **Error Handling** - Route failed requests through an explicit error path
...
The API block does not automatically retry timeouts, connection failures, HTTP 429, or 5xx responses. Failed requests raise a block error. Connect the block's error path to recovery logic; implement bounded retries and backoff explicitly in the workflow when needed. Only retry requests that are safe to repeat.

### Response Validation

Connect a Function block to the API block's normal output to validate a successful response. Connect failure recovery to the API block's error path, not a Condition after its normal output. Set the API URL to `<function.result>` when using the URL-building Function above.
