...
<Tabs items={['For', 'ForEach', 'While', 'Do-While']}>
  ...
  <Tab>
    **While Loop** -- evaluates a condition **before** each iteration. If the condition is false on the first check, the body never executes.
    ...
    While and Do-While have no separate per-loop iteration cap, but execution is bounded by the workflow timeout and the executor's 500 scheduling-pass limit. A scheduling pass is not a loop iteration; the number of completed iterations depends on the workflow graph.
  </Tab>
  <Tab>
...
- **Always update your condition variable** inside While/Do-While loops. Forgetting to change the variable the condition depends on creates an infinite loop (bounded by the executor scheduling limit and workflow timeout).
