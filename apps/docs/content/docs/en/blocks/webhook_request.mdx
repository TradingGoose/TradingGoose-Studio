...
1. Resolves exact source-qualified references such as `<agent.content>` and workflow variables such as `<variable.userId>` in the URL, payload, and headers. Do not invent generic block-prefixed placeholders.
...
| **Payload** | No | JSON body to send. Use an exact source-qualified tag such as `<agent.content>` or a workflow variable such as `<variable.userId>`. |
