...
<Callout type="info">
  The `trigger()` API is a TradingGoose-specific extension. It does not exist in standard Pine Script or PineTS. Stock PineTS throws `trigger is not defined` when it reaches these calls. Remove or adapt them before running an exported indicator outside TradingGoose.
</Callout>
