...
| `math.round_to_mintick(x)` | Unavailable in Studio: requires `syminfo.mintick`; round using an explicit input instead |
| `math.sign(x)` | Sign |
...
| `hline(price, title?, color?, linestyle?, linewidth?, editable?, display?)` | Horizontal line |
| `fill(plot1, plot2, color?)` | Fill between plots |
| `bgcolor(color, offset?, options?)` | Not rendered in Studio; produces an unsupported-style warning |
| `barcolor(color, offset?, options?)` | Not rendered in Studio; use a plot's `color` option instead |
...
| **Stats** | `sum`, `avg`, `min`, `max`, `median`, `mode` |
...
`request.security` and `request.security_lower_tf` are not supported in TradingGoose. Indicator verification and server compilation reject these calls. Use the chart's supplied data or fetch the required series in a workflow instead.
