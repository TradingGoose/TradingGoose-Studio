...
| `hline(price, title?, color?, linestyle?, linewidth?, editable?, display?)` | Draw a horizontal line at a fixed price level |
...
PineTS accepts these constants, but Studio currently does not preserve `linestyle` when rendering plots. Dashed and dotted styles render as solid lines.
...
Studio does not render `bgcolor()` or `barcolor()` output; these calls produce unsupported-style warnings. To visualize a condition, set the `color` option on a supported `plot()` or `plotshape()` instead.
