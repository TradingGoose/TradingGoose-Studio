...
Studio runs PineTS with supplied OHLCV bars and does not populate `syminfo`. Accessing properties such as `syminfo.mintick` or `syminfo.ticker` will fail. Use explicit indicator inputs for required symbol-specific values, such as a minimum calculation increment.
