...
Once your documents are processed, add a Knowledge block, select a knowledgebase, and configure a search query. To use the results for Retrieval-Augmented Generation (RAG), explicitly reference the block's results in the downstream Agent prompt, for example `<knowledge.results>` when the block is named `knowledge`. Connecting the blocks alone does not inject retrieved content. You can also configure Knowledge as an Agent tool for the agent to invoke.

**Current tag-filter limitation:** The Knowledge block does not currently send filters in the format accepted by the search API. Tag-only requests fail, and requests with a query and tags run without those tag filters. Do not rely on block tag filters to restrict retrieved content. See [Tags and Filtering](/knowledgebase/tags) for the API's supported search modes.
