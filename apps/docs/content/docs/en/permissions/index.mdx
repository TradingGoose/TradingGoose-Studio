...
To invite someone, navigate to your workspace settings and select the **Members** tab. Click **Invite** to add a new member by email, then choose their permission level (Read, Write, or Admin).
...
- Delete the workspace, provided it is not their last accessible workspace
...
### Workspace Admin  
- Has the same workspace-management permissions as the owner, including workspace deletion
- Cannot remove the workspace owner
- Can be removed from the workspace by the owner or other admins, unless they are its configured personal billing owner

### Personal Billing Owner
- Billing ownership is separate from workspace ownership
- When a workspace is billed to a user, that user must retain Admin permission and cannot be removed
- Reassign billing before removing that user or reducing their permission below Admin

Owners and admins cannot delete their last accessible workspace. TradingGoose does not currently provide a workspace-ownership transfer action.
...
- If a personal variable has the same name as a workspace variable, the workspace variable takes precedence
...
- View the organization's billed-workspace list
- Assign or release organization billing for workspaces where they also have workspace Admin access

**What they cannot do:**
- Access workspace content unless they own that workspace or have been granted workspace access
- Change the organization's subscription, seat count, or usage limit

Only the **Organization Owner** can change the organization's subscription, seat count, or usage limit. Organization Admins can manage which eligible workspaces use organization billing, but the organization role alone does not grant access to workspace content.
