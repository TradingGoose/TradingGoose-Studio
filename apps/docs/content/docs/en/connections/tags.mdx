...
- `blockName` is the name of the source block (or its ID)
- `path.to.data` is the dot-separated path to the specific data field

For example:

- `<agent1.content>` - References the content field from a block named "agent1"
- `<api2.data.users[0].name>` - References the name of the first user in the users array from the data field of a block named "api2"

### Special Reference Types

In addition to block references, the resolver supports several special prefixes:

- **Trigger aliases**: `<start.input>`, `<api.fieldName>`, `<chat.input>`, `<manual.input>` - shorthand references to trigger block outputs
- **Variables**: `<variable.myVar>` - references workflow variables by name
- **Loops**: `<loop.item>`, `<loop.index>` - references the current item or index within a loop
- **Parallels**: `<parallel.item>`, `<parallel.index>` - references the current item or index within a parallel execution

## Dynamic Tag References

Connection tags are evaluated at runtime, which means:

1. They always reference the most current data from the execution context
2. They can be used in expressions and combined with static text
3. They can be nested within other data structures
4. Array indexing is supported using bracket notation (e.g., `items[0]`)
