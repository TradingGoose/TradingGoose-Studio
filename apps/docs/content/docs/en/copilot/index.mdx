...
Copilot is an AI-powered assistant that works across the entire TradingGoose workspace — not just workflows. It can manage workflows, MCP servers, skills, custom tools, knowledge bases, deployments, variables, and more. Open or close its persistent workspace panel with the **Copilot** control in the global sidebar.
...
Models are grouped by capability tier (fast, standard, reasoning). Enabled models sync across the Copilot panel and settings so your selection is consistent everywhere.
...
Chat state persists while you navigate the workspace or collapse and reopen the panel. Copilot follows the current page and selection: for example, a knowledge detail, selected execution log, or monitor. On the dashboard, you can choose a pair color from the Copilot header to include the active layout and the entities published by widgets using that color. Dashboard pair colors do not apply on other pages.

## Editor Integration

Copilot is context-aware. On the dashboard, select the same pair color as an editor widget to include its active context (workflow, indicator, skill, MCP server, or custom tool) when sending messages. On other pages, Copilot uses the page's current route or selection context instead of dashboard pairing.
