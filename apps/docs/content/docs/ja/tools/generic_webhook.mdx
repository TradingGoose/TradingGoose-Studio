...
Generic Webhookブロックは、外部サービスからのWebhookを受信することができます。これはあらゆるJSONペイロードを処理できる柔軟なトリガーであり、専用のTradingGooseブロックを持たないサービスとの統合に最適です。
...
2. シークレットトークンを設定する
3. ヘッダータイプを選択する：
   - **カスタムヘッダー**：`X-TradingGoose-Secret: your-token`
