...
TradingGooseの実行エンジンは、ブロックを正しい順序で処理し、データフローを管理し、エラーを適切に処理することで、ワークフローに命を吹き込みます。これにより、TradingGooseでワークフローがどのように実行されるかを正確に理解することができます。
...
ワークフローをAPIとしてデプロイすると、TradingGooseは以下を行います：
- 一意のHTTPエンドポイントを作成: `https://tradinggoose.ai/api/workflows/{workflowId}/execute`
- 認証用のAPIキーを生成
...
- カスタムサブドメインでホスト: `https://your-name.tradinggoose.ai`
