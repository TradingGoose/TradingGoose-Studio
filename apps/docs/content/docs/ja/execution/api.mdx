...
TradingGooseは、ワークフローの実行ログを照会したり、ワークフローが完了したときにリアルタイム通知を設定するためのWebhookを設定したりするための包括的な外部APIを提供しています。
...
TradingGooseダッシュボードのユーザーセッティングからAPIキーを生成できます。
...
ワークフロー実行が完了したときにリアルタイム通知を受け取ります。WebhookはTradingGoose UIのワークフローエディタで設定されます。
...
ワークフロー実行が完了すると、TradingGooseはWebhook URLにPOSTリクエストを送信します:
...
- `tradinggoose-event`: イベントタイプ（常に `workflow.execution.completed`）
- `tradinggoose-timestamp`: ミリ秒単位のUnixタイムスタンプ
- `tradinggoose-delivery-id`: べき等性のための一意の配信ID
- `tradinggoose-signature`: 検証用のHMAC-SHA256署名（シークレットが設定されている場合）
- `Idempotency-Key`: 重複検出のための配信IDと同じ値

### 署名検証

Webhookシークレットを設定した場合、署名を検証してWebhookがTradingGooseからのものであることを確認してください：
...
2. **Webhookセキュリティ**: 常にWebhookシークレットを設定し、署名を検証してリクエストがTradingGooseからのものであることを確認してください。
