...
TradingGooseの公式Python SDKを使用すると、公式Python SDKを使用してPythonアプリケーションからプログラムでワークフローを実行できます。
...
**パラメータ：**
- `api_key` (str): TradingGooseのAPIキー
- `base_url` (str, オプション): TradingGoose APIのベースURL
