...
TradingGoose bietet eine umfassende externe API zum Abfragen von Workflow-Ausführungsprotokollen und zum Einrichten von Webhooks für Echtzeit-Benachrichtigungen, wenn Workflows abgeschlossen werden.
...
Sie können API-Schlüssel in Ihren Benutzereinstellungen im TradingGoose-Dashboard generieren.
...
Erhalten Sie Echtzeitbenachrichtigungen, wenn Workflow-Ausführungen abgeschlossen werden. Webhooks werden über die TradingGoose-Benutzeroberfläche im Workflow-Editor konfiguriert.
...
Wenn eine Workflow-Ausführung abgeschlossen ist, sendet TradingGoose eine POST-Anfrage an Ihre Webhook-URL:
...
- `tradinggoose-event`: Ereignistyp (immer `workflow.execution.completed`)
- `tradinggoose-timestamp`: Unix-Zeitstempel in Millisekunden
- `tradinggoose-delivery-id`: Eindeutige Lieferungs-ID für Idempotenz
- `tradinggoose-signature`: HMAC-SHA256-Signatur zur Verifizierung (falls Secret konfiguriert)
- `Idempotency-Key`: Identisch mit der Lieferungs-ID zur Erkennung von Duplikaten

### Signaturverifizierung

Wenn Sie ein Webhook-Secret konfigurieren, überprüfen Sie die Signatur, um sicherzustellen, dass der Webhook von TradingGoose stammt:
...
2. **Webhook-Sicherheit**: Konfigurieren Sie immer ein Webhook-Secret und überprüfen Sie Signaturen, um sicherzustellen, dass Anfragen von TradingGoose stammen.
