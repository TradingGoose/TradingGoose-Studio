  ...
  'relative mt-1 flex w-full items-center gap-2  h-8 overflow-hidden rounded-md p-2 text-left text-sm font-sm text-fd-foreground [overflow-wrap:anywhere] transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fd-ring [&_svg]:size-4 [&_svg]:shrink-0 data-[collapsed=true]:justify-center data-[collapsed=true]:ml-1 data-[collapsed=true]:px-2 data-[collapsed=true]:w-8 data-[collapsed=true]:py-2',
  {
    variants: {
      active: {
        ...
        false: 'hover:bg-fd-secondary hover:text-fd-foreground',
      },
    },
  }
)
...
export function SidebarContent(props: ComponentProps<'aside'>) {
  ...
  useEffect(() => {
    ...
    if (!match) return
    const cookieWidth = sidebarWidthFromString(decodeURIComponent(match[1]), DEFAULT_WIDTH)
...
export function SidebarHeader(props: ComponentProps<'div'>) {
  ...
  const collapsedState = dataCollapsed !== undefined ? dataCollapsed : collapsed
...
export function SidebarItem({
  ...
  const { collapsed } = useSidebar()
  const itemLabel = typeof props.children === 'string' ? props.children : undefined
  ...
  return (
    ...
    >
      ...
      <span className={cn('truncate', collapsed && 'sr-only')}>{props.children}</span>
    </Link>
  )
}
...
export function SidebarFolderTrigger({
  ...
  return (
    <CollapsibleTrigger
      className={cn(itemVariants({ active: false }), className)}
      {...props}
      data-collapsed={collapsed}
