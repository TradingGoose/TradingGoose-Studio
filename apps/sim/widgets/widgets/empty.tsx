...
const EmptyBody = ({ widget, onWidgetChange }: EmptyWidgetProps) => (
  <Empty className='border border-dashed border-border   p-6'>
    <EmptyHeader>
      ...
      <EmptyTitle>{widget?.key && widget?.key !== 'empty' ? 'Empty Widget' : 'No widget selected'}</EmptyTitle>
      <EmptyDescription>
...
export const emptyWidget: DashboardWidgetDefinition = {
  ...
  component: EmptyBody,
  renderHeader: () => ({
    center: <span className='text-xs text-muted-foreground'>No widget selected</span>,
  }),
}
