...
const WorkflowConsoleWidgetBody = ({
  ...
  const widgetKey = widget?.key ?? 'workflow-console'
  ...
  useEffect(() => {
    ...
    loadWorkflows(workspaceId)
      .catch((error) => {
        if (!cancelled) {
          console.error('Failed to load workflows for workflow console widget', error)
  ...
  useEffect(() => {
    ...
    setActiveWorkflow({ workflowId: resolvedWorkflowId, channelId })
      .catch((error) => {
        if (!cancelled) {
          console.error('Failed to activate workflow inside console widget', error)
        }
      })
