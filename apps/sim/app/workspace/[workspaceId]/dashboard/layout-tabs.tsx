    ...
    >
      <div className='flex min-w-0 items-center gap-2'>
        <div className='flex min-w-0 flex-1 items-center gap-2 rounded-md bg-muted px-1 py-1'>
          <div
            ...
            className='flex flex-1 min-w-0 items-center overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden'
          >
              ...
              {layouts.map((layout) => (
                ...
                >
                  ...
                  {editingId === layout.id ? (
                    ...
                    <button
                      type='button'
                      className='inline-flex h-full items-center gap-2 px-1 text-sm font-medium outline-none transition-colors'
                      onClick={() => onSelect(layout.id)}
                      disabled={isBusy}
                      tabIndex={-1}
                      onPointerDownCapture={(event) => event.stopPropagation()}
                    >
                      <span className='truncate text-md font-md pb-1'>{layout.name}</span>
                    </button>
                  )}
                  {editingId === layout.id ? (
                    <button
        ...
        <button
          type='button'
          className={cn(
            'inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-sm border border-border   text-muted-foreground transition  focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 hover:bg-card hover:text-foreground',
      ...
      <SortableOverlay>
        {({ value }) => {
          ...
          return (
            <div className='inline-flex items-stretch overflow-hidden py-1.5 gap-1 rounded-sm border border-border bg-background text-foreground shadow-md'>
              <div className='inline-flex items-center justify-center pl-2 '>
                <Move className='h-3.5 w-3.5' />
              </div>
              <div className='inline-flex items-center  pr-3 px-1  text-sm'>
